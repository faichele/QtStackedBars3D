...
namespace QtStackedBar3DVis
{
	...
	QScatterRenderer3D::QScatterRenderer3D(QScatterController3D *controller)
		...
		m_scaleY(0.0f),
		m_scaleZ(0.0f),
		m_selectedItemIndex(QScatterController3D::invalidSelectionIndex()),
		m_selectedSeriesCache(0),
		m_oldSelectedSeriesCache(0),
		m_dotSizeScale(1.0f),
		m_maxItemSize(0.0f),
		m_clickedIndex(QScatterController3D::invalidSelectionIndex()),
	...
	QScatterRenderer3D::~QScatterRenderer3D()
	{
	...
	void QScatterRenderer3D::initializeOpenGL()
	{
	...
	void QScatterRenderer3D::fixCameraTarget(QVector3D &target)
	{
	...
	void QScatterRenderer3D::getVisibleItemBounds(QVector3D &minBounds, QVector3D &maxBounds)
	{
	...
	void QScatterRenderer3D::updateData()
	{
		calculateSceneScalingFactors();
		int totalDataSize = 0;

		foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
			QScatterSeriesRenderCache3D *cache = static_cast<QScatterSeriesRenderCache3D *>(baseCache);
			if (cache->isVisible()) {
				const QScatterSeries3D *currentSeries = cache->series();
				QScatterRenderItemArray3D &renderArray = cache->renderArray();
				QScatterDataProxy3D *dataProxy = currentSeries->dataProxy();
		...
		if (m_cachedOptimizationHint.testFlag(QAbstractGraph3D::OptimizationStatic)) {
			foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
				...
				if (cache->isVisible()) {
					...
					if (cache->mesh() == QAbstractSeries3D::MeshPoint) {
						QScatterPointBufferHelper3D *points = cache->bufferPoints();
						if (!points) {
							points = new QScatterPointBufferHelper3D();
						...
						QScatterObjectBufferHelper3D *object = cache->bufferObject();
						if (!object) {
							object = new QScatterObjectBufferHelper3D();
	...
	void QScatterRenderer3D::updateSeries(const QList<QAbstractSeries3D *> &seriesList)
	{
		...
		if (m_cachedOptimizationHint.testFlag(QAbstractGraph3D::OptimizationStatic)) {
			for (int i = 0; i < seriesCount; i++) {
				QScatterSeries3D *scatterSeries = static_cast<QScatterSeries3D *>(seriesList[i]);
				if (scatterSeries->isVisible()) {
					QAbstractSeriesChangeBitField3D &changeTracker = scatterSeries->d_ptr->m_changeTracker;
					QScatterSeriesRenderCache3D *cache =
						static_cast<QScatterSeriesRenderCache3D *>(m_renderCacheList.value(scatterSeries));
					if (cache) {
		...
		for (int i = 0; i < seriesCount; i++) {
			...
			if (scatterSeries->isVisible()) {
				...
				if (noSelection
					&& scatterSeries->selectedItem() != QScatterSeries3D::invalidSelectionIndex()) {
				...
				if (cache->staticObjectUVDirty()) {
					if (cache->mesh() == QAbstractSeries3D::MeshPoint) {
						QScatterPointBufferHelper3D *object = cache->bufferPoints();
	...
	QSeriesRenderCache3D *QScatterRenderer3D::createNewCache(QAbstractSeries3D *series)
	{
		return new QScatterSeriesRenderCache3D(series, this);
	}

	void QScatterRenderer3D::updateItems(const QVector<QScatterController3D::ChangeItem> &items)
	{
		QScatterSeriesRenderCache3D *cache = 0;
		const QScatterSeries3D *prevSeries = 0;
		const QScatterDataArray *dataArray = 0;
		const bool optimizationStatic = m_cachedOptimizationHint.testFlag(
			QAbstractGraph3D::OptimizationStatic);

		foreach(QScatterController3D::ChangeItem item, items) {
			QScatterSeries3D *currentSeries = item.series;
			if (currentSeries != prevSeries) {
				cache = static_cast<QScatterSeriesRenderCache3D *>(m_renderCacheList.value(currentSeries));
			...
			if (cache->isVisible()) {
				...
				bool oldVisibility;
				QScatterRenderItem3D &item = cache->renderArray()[index];
			...
			foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
				...
				if (cache->isVisible() && cache->updateIndices().size()) {
					if (cache->mesh() == QAbstractSeries3D::MeshPoint) {
						...
						if (cache->colorStyle() == QTheme3D::ColorStyleRangeGradient)
	...
	void QScatterRenderer3D::updateScene(QScene3D *scene)
	{
	...
	void QScatterRenderer3D::updateAxisLabels(QAbstractAxis3D::AxisOrientation orientation,
	...
	void QScatterRenderer3D::updateAxisTitleVisibility(QAbstractAxis3D::AxisOrientation orientation,
		bool visible)
	{
	...
	void QScatterRenderer3D::updateOptimizationHint(QAbstractGraph3D::OptimizationHints hint)
	{
		QAbstractRenderer3D::updateOptimizationHint(hint);

		QAbstractRenderer3D::reInitShaders();

		if (m_isOpenGLES && hint.testFlag(QAbstractGraph3D::OptimizationStatic)
	...
	void QScatterRenderer3D::updateMargin(float margin)
	{
		QAbstractRenderer3D::updateMargin(margin);
		calculateSceneScalingFactors();
	}

	void QScatterRenderer3D::resetClickedStatus()
	{
		m_clickedIndex = QScatterController3D::invalidSelectionIndex();
		m_clickedSeries = 0;
	}

	void QScatterRenderer3D::render(GLuint defaultFboHandle)
	{
	...
	void QScatterRenderer3D::drawScene(const GLuint defaultFboHandle)
	{
		...
		const bool optimizationDefault =
			!m_cachedOptimizationHint.testFlag(QAbstractGraph3D::OptimizationStatic);
		...
		QShaderHelper3D *pointSelectionShader;
		if (!m_isOpenGLES) {
			...
			if (m_cachedShadowQuality > QAbstractGraph3D::ShadowQualityNone) {
				...
				foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
					if (baseCache->isVisible()) {
						QScatterSeriesRenderCache3D *cache =
							static_cast<QScatterSeriesRenderCache3D *>(baseCache);
						QObjectHelper3D *dotObj = cache->object();
						QQuaternion seriesRotation(cache->meshRotation());
						const QScatterRenderItemArray3D &renderArray = cache->renderArray();
						const int renderArraySize = renderArray.size();
						bool drawingPoints = (cache->mesh() == QAbstractSeries3D::MeshPoint);
						...
						for (int dot = 0; dot < loopCount; dot++) {
							const QScatterRenderItem3D &item = renderArray.at(dot);
		...
		QShaderHelper3D *selectionShader = m_selectionShader;
		...
		if (m_cachedSelectionMode > QAbstractGraph3D::SelectionNone
			...
			QVector4D clickedColor = QUtils3D::getSelection(m_inputPosition,
				m_viewport.height());
		...
		// Draw dots
		QShaderHelper3D *dotShader = 0;
		GLuint gradientTexture = 0;
		bool dotSelectionFound = false;
		QScatterRenderItem3D *selectedItem(0);
		QVector4D baseColor;
		QVector4D dotColor;

		bool previousDrawingPoints = false;
		QTheme3D::ColorStyle previousMeshColorStyle = QTheme3D::ColorStyleUniform;
		if (m_haveMeshSeries) {
			...
			if (m_haveUniformColorMeshSeries) {
				...
				previousMeshColorStyle = QTheme3D::ColorStyleRangeGradient;
		...
		foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
			if (baseCache->isVisible()) {
				...
				QScatterRenderItemArray3D &renderArray = cache->renderArray();
				const int renderArraySize = renderArray.size();
				bool selectedSeries = m_cachedSelectionMode > QAbstractGraph3D::SelectionNone
				...
				bool rangeGradientPoints = drawingPoints
					&& (colorStyle == QTheme3D::ColorStyleRangeGradient);
				float itemSize = cache->itemSize() / itemScaler;
				if (itemSize == 0.0f)
					itemSize = m_dotSizeScale;
	#if !defined(QT_OPENGL_ES_2)
				...
				if (!drawingPoints && !colorStyleIsUniform && previousMeshColorStyle != colorStyle) {
					if (colorStyle == QTheme3D::ColorStyleObjectGradient) {
				...
				for (int i = 0; i < loopCount; i++) {
					QScatterRenderItem3D &item = renderArray[i];
					if (!item.isVisible() && optimizationDefault)
						continue;
					...
					if (useColor) {
						if (rangeGradientPoints) {
							...
							dotColor = QUtils3D::vectorFromColor(
				...
				if (!optimizationDefault && selectedSeries
					&& m_selectedItemIndex != QScatterController3D::invalidSelectionIndex()) {
					QScatterRenderItem3D &item = renderArray[m_selectedItemIndex];
					if (item.isVisible()) {
						QShaderHelper3D *selectionShader;
						if (drawingPoints) {
							selectionShader = pointSelectionShader;
						}
						else {
	...
	void QScatterRenderer3D::drawLabels(bool drawSelection, const QCamera3D *activeCamera,
		...
		if (m_axisCacheZ.segmentCount() > 0) {
			...
			for (int label = startIndex; label != endIndex; label = label + indexStep) {
				...
				m_drawer->drawLabel(m_dummyRenderItem, axisLabelItem, viewMatrix, projectionMatrix,
					...
					QDrawer3D::LabelMid, alignment, false, drawSelection);
		...
		if (m_axisCacheY.segmentCount() > 0) {
			...
			for (int label = startIndex; label != endIndex; label = label + indexStep) {
				...
				m_drawer->drawLabel(m_dummyRenderItem, axisLabelItem, viewMatrix, projectionMatrix,
					...
					shader, m_labelObj, activeCamera, true, true,
					QDrawer3D::LabelMid, backAlignment, false, drawSelection);
				...
				m_drawer->drawLabel(m_dummyRenderItem, axisLabelItem, viewMatrix, projectionMatrix,
					...
					shader, m_labelObj, activeCamera, true, true,
					QDrawer3D::LabelMid, sideAlignment, false, drawSelection);
				labelsMaxWidth = qMax(labelsMaxWidth, float(axisLabelItem.size().width()));
			}
	...
	void QScatterRenderer3D::updateSelectedItem(int index, QScatterSeries3D *series)
	{
		m_selectionDirty = true;
		m_selectionLabelDirty = true;
		m_selectedSeriesCache =
			static_cast<QScatterSeriesRenderCache3D *>(m_renderCacheList.value(series, 0));
		m_selectedItemIndex = QScatterController3D::invalidSelectionIndex();

		if (m_cachedOptimizationHint.testFlag(QAbstractGraph3D::OptimizationStatic)
			&& m_oldSelectedSeriesCache
			&& m_oldSelectedSeriesCache->mesh() == QAbstractSeries3D::MeshPoint) {
			m_oldSelectedSeriesCache->bufferPoints()->popPoint();
			m_oldSelectedSeriesCache = 0;
		}

		if (m_selectedSeriesCache) {
			const QScatterRenderItemArray3D &renderArray = m_selectedSeriesCache->renderArray();
			if (index < renderArray.size() && index >= 0) {
				m_selectedItemIndex = index;

				if (m_cachedOptimizationHint.testFlag(QAbstractGraph3D::OptimizationStatic)
					&& m_selectedSeriesCache->mesh() == QAbstractSeries3D::MeshPoint) {
	...
	void QScatterRenderer3D::updateShadowQuality(QAbstractGraph3D::ShadowQuality quality)
	{
	...
	void QScatterRenderer3D::fixMeshFileName(QString &fileName, QAbstractSeries3D::Mesh mesh)
	{
		// Load full version of meshes that have it available
		if (mesh != QAbstractSeries3D::MeshSphere
			&& mesh != QAbstractSeries3D::MeshMinimal
			&& mesh != QAbstractSeries3D::MeshPoint
			&& mesh != QAbstractSeries3D::MeshArrow) {
			fileName.append(QStringLiteral("Full"));
		}
	}

	void QScatterRenderer3D::calculateTranslation(QScatterRenderItem3D &item)
	{
	...
	void QScatterRenderer3D::calculateSceneScalingFactors()
	{
	...
	void QScatterRenderer3D::initShaders(const QString &vertexShader, const QString &fragmentShader)
	{
		delete m_dotShader;
		m_dotShader = new QShaderHelper3D(this, vertexShader, fragmentShader);
		m_dotShader->initialize();
	}

	void QScatterRenderer3D::initGradientShaders(const QString &vertexShader,
		const QString &fragmentShader)
	{
		delete m_dotGradientShader;
		m_dotGradientShader = new QShaderHelper3D(this, vertexShader, fragmentShader);
		m_dotGradientShader->initialize();

	}

	void QScatterRenderer3D::initStaticSelectedItemShaders(const QString &vertexShader,
		const QString &fragmentShader,
		const QString &gradientVertexShader,
		const QString &gradientFragmentShader)
	{
		delete m_staticSelectedItemShader;
		m_staticSelectedItemShader = new QShaderHelper3D(this, vertexShader, fragmentShader);
		m_staticSelectedItemShader->initialize();

		delete m_staticSelectedItemGradientShader;
		m_staticSelectedItemGradientShader = new QShaderHelper3D(this, gradientVertexShader,
			gradientFragmentShader);
		m_staticSelectedItemGradientShader->initialize();
	}

	void QScatterRenderer3D::initSelectionShader()
	{
		delete m_selectionShader;
	...
	void QScatterRenderer3D::initSelectionBuffer()
	{
		m_textureHelper->deleteTexture(&m_selectionTexture);
	...
	void QScatterRenderer3D::initDepthShader()
	{
		if (!m_isOpenGLES) {
			if (m_depthShader)
				delete m_depthShader;
	...
	void QScatterRenderer3D::updateDepthBuffer()
	{
		if (!m_isOpenGLES) {
	...
	void QScatterRenderer3D::initPointShader()
	{
		if (m_isOpenGLES) {
			if (m_pointShader)
				delete m_pointShader;
			m_pointShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexPointES2"),
				QStringLiteral(":/shaders/fragmentPlainColor"));
			m_pointShader->initialize();
		}
	}

	void QScatterRenderer3D::initBackgroundShaders(const QString &vertexShader,
		const QString &fragmentShader)
	{
	...
	void QScatterRenderer3D::initStaticPointShaders(const QString &vertexShader,
		const QString &fragmentShader)
	{
		if (m_staticGradientPointShader)
			delete m_staticGradientPointShader;
		m_staticGradientPointShader = new QShaderHelper3D(this, vertexShader, fragmentShader);
		m_staticGradientPointShader->initialize();
	}

	void QScatterRenderer3D::selectionColorToSeriesAndIndex(const QVector4D &color,
		int &index,
		QAbstractSeries3D *&series)
	{
		...
		if (color != selectionSkipColor) {
			if (color.w() == labelRowAlpha) {
				// Row selection
				index = QScatterController3D::invalidSelectionIndex();
	...
	void QScatterRenderer3D::updateRenderItem(const QScatterDataItem3D &dataItem,
		QScatterRenderItem3D &renderItem)
	{
	...
	QVector3D QScatterRenderer3D::convertPositionToTranslation(const QVector3D &position,
		bool isAbsolute)
	{
		float xTrans = 0.0f;
		float yTrans = 0.0f;
