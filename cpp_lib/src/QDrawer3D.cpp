...
namespace QtStackedBar3DVis
{
	...
	QDrawer3D::QDrawer3D(QTheme3D *theme)
		: m_theme(theme),
		m_textureHelper(0),
		m_pointbuffer(0),
		m_linebuffer(0),
	...
	void QDrawer3D::initializeOpenGL()
	{
	...
	void QDrawer3D::setTheme(QTheme3D *theme)
	{
		m_theme = theme;
	...
	QTheme3D *QDrawer3D::theme() const
	{
		return m_theme;
	}

	QFont QDrawer3D::font() const
	{
		return m_theme->font();
	}

	void QDrawer3D::drawObject(QShaderHelper3D *shader, QAbstractObjectHelper3D *object, GLuint textureId,
	...
	void QDrawer3D::drawSelectionObject(QShaderHelper3D *shader, QAbstractObjectHelper3D *object)
	{
		glEnableVertexAttribArray(shader->posAtt());
		glBindBuffer(GL_ARRAY_BUFFER, object->vertexBuf());
		glVertexAttribPointer(shader->posAtt(), 3, GL_FLOAT, GL_FALSE, 0, (void *)0);
	...
	void QDrawer3D::drawSurfaceGrid(QShaderHelper3D *shader, QSurfaceObject3D *object)
	{
		// 1st attribute buffer : vertices
		glEnableVertexAttribArray(shader->posAtt());
		glBindBuffer(GL_ARRAY_BUFFER, object->vertexBuf());
	...
	void QDrawer3D::drawPoint(QShaderHelper3D *shader)
	{
		// Draw a single point
	...
	void QDrawer3D::drawPoints(QShaderHelper3D *shader, QScatterPointBufferHelper3D *object, GLuint textureId)
	{
		if (textureId) {
			// Activate texture
			glActiveTexture(GL_TEXTURE0);
	...
	void QDrawer3D::drawLine(QShaderHelper3D *shader)
	{
		// Draw a single line
	...
	void QDrawer3D::drawLabel(const QAbstractRenderItem3D &item, const QLabelItem3D &labelItem,
		const QMatrix4x4 &viewmatrix, const QMatrix4x4 &projectionmatrix,
		const QVector3D &positionComp, const QQuaternion &rotation,
		GLfloat itemHeight, QAbstractGraph3D::SelectionFlags mode,
		QShaderHelper3D *shader, QObjectHelper3D *object,
		const QCamera3D *camera, bool useDepth, bool rotateAlong,
		LabelPosition position, Qt::AlignmentFlag alignment, bool isSlicing,
		bool isSelecting, bool smallFontSize)
	{
		// Draw label
		...
		if (position < LabelBottom) {
			...
			else if (mode.testFlag(QAbstractGraph3D::SelectionColumn) && isSlicing)
	...
	void QDrawer3D::generateSelectionLabelTexture(QAbstractRenderer3D *renderer)
	{
		QLabelItem3D &labelItem = renderer->selectionLabelItem();
		generateLabelItem(labelItem, renderer->selectionLabel());
	}

	void QDrawer3D::generateLabelItem(QLabelItem3D &item, const QString &text, int widestLabel)
	{
		initializeOpenGL();
		...
		if (!text.isEmpty()) {
			...
			QImage label = QUtils3D::printTextToImage(m_theme->font(),
				text,
