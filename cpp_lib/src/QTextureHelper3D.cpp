...
namespace QtStackedBar3DVis
{
	...
	QTextureHelper3D::QTextureHelper3D()
	{
	...
	GLuint QTextureHelper3D::create2DTexture(const QImage &image, bool useTrilinearFiltering,
		bool convert, bool smoothScale, bool clampY)
	{
		...
		if (QUtils3D::isOpenGLES()) {
			GLuint imageWidth = QUtils3D::getNearestPowerOfTwo(image.width());
			GLuint imageHeight = QUtils3D::getNearestPowerOfTwo(image.height());
			if (smoothScale) {
	...
	GLuint QTextureHelper3D::create3DTexture(const QVector<uchar> *data, int width, int height, int depth,
		QImage::Format dataFormat)
	{
		if (QUtils3D::isOpenGLES() || !width || !height || !depth)
			return 0;
	...
	GLuint QTextureHelper3D::createCubeMapTexture(const QImage &image, bool useTrilinearFiltering)
	{
		if (image.isNull())
			return 0;
	...
	GLuint QTextureHelper3D::createSelectionTexture(const QSize &size, GLuint &frameBuffer,
		GLuint &depthBuffer)
	{
		GLuint textureid;
	...
	GLuint QTextureHelper3D::createCursorPositionTexture(const QSize &size, GLuint &frameBuffer)
	{
		GLuint textureid;
		glGenTextures(1, &textureid);
		glBindTexture(GL_TEXTURE_2D, textureid);
	...
	GLuint QTextureHelper3D::createUniformTexture(const QColor &color)
	{
	...
	GLuint QTextureHelper3D::createGradientTexture(const QLinearGradient &gradient)
	{
	...
	GLuint QTextureHelper3D::createDepthTexture(const QSize &size, GLuint textureSize)
	{
	...
	GLuint QTextureHelper3D::createDepthTextureFrameBuffer(const QSize &size, GLuint &frameBuffer,
		GLuint textureSize)
	{
	...
	void QTextureHelper3D::deleteTexture(GLuint *texture)
	{
		if (texture && *texture) {
	...
	QImage QTextureHelper3D::convertToGLFormat(const QImage &srcImage)
	{
	...
	void QTextureHelper3D::convertToGLFormatHelper(QImage &dstImage, const QImage &srcImage,
	...
	QRgb QTextureHelper3D::qt_gl_convertToGLFormatHelper(QRgb src_pixel, GLenum texture_format)
	{
#if !defined(QT_OPENGL_ES_2)
		if (texture_format == GL_BGRA) {
#else
