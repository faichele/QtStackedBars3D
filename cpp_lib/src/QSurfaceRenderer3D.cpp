...
namespace QtStackedBar3DVis
{
	...
	QSurfaceRenderer3D::QSurfaceRenderer3D(QSurfaceController3D *controller)
		...
		m_selectedSeries(0),
		m_clickedPosition(QSurfaceController3D::invalidSelectionPosition()),
		m_selectionTexturesDirty(false),
		m_noShadowTexture(0)
	{
		// Check if flat feature is supported
		QShaderHelper3D tester(this, QStringLiteral(":/shaders/vertexSurfaceFlat"),
			QStringLiteral(":/shaders/fragmentSurfaceFlat"));
		if (!tester.testCompile()) {
			m_flatSupported = false;
			connect(this, &QSurfaceRenderer3D::flatShadingSupportedChanged,
				controller, &QSurfaceController3D::handleFlatShadingSupportedChange);
	...
	QSurfaceRenderer3D::~QSurfaceRenderer3D()
	{
	...
	void QSurfaceRenderer3D::initializeOpenGL()
	{
	...
	void QSurfaceRenderer3D::fixCameraTarget(QVector3D &target)
	{
	...
	void QSurfaceRenderer3D::getVisibleItemBounds(QVector3D &minBounds, QVector3D &maxBounds)
	{
	...
	void QSurfaceRenderer3D::updateData()
	{
		calculateSceneScalingFactors();

		foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
			QSurfaceSeriesRenderCache3D *cache = static_cast<QSurfaceSeriesRenderCache3D *>(baseCache);
			if (cache->isVisible() && cache->dataDirty()) {
				const QSurfaceSeries3D *currentSeries = cache->series();
				QSurfaceDataProxy3D *dataProxy = currentSeries->dataProxy();
				const QSurfaceDataArray3D &array = *dataProxy->array();
				QSurfaceDataArray3D &dataArray = cache->dataArray();
				QRect sampleSpace;
				...
				if (sampleSpace.width() >= 2 && sampleSpace.height() >= 2) {
					if (dimensionsChanged) {
						...
						for (int i = 0; i < sampleSpace.height(); i++)
							dataArray << new QSurfaceDataRow3D(sampleSpace.width());
					}
					for (int i = 0; i < sampleSpace.height(); i++) {
		...
		if (m_selectionTexturesDirty && m_cachedSelectionMode > QAbstractGraph3D::SelectionNone)
			updateSelectionTextures();

		updateSelectedPoint(m_selectedPoint, m_selectedSeries);
	}

	void QSurfaceRenderer3D::updateSeries(const QList<QAbstractSeries3D *> &seriesList)
	{
		QAbstractRenderer3D::updateSeries(seriesList);

		bool noSelection = true;
		foreach(QAbstractSeries3D *series, seriesList) {
			QSurfaceSeries3D *surfaceSeries = static_cast<QSurfaceSeries3D *>(series);
			QSurfaceSeriesRenderCache3D *cache =
				static_cast<QSurfaceSeriesRenderCache3D *>(m_renderCacheList.value(series));
			if (noSelection
				&& surfaceSeries->selectedPoint() != QSurfaceSeries3D::invalidSelectionPosition()) {
		...
		if (noSelection && !selectionLabel().isEmpty()) {
			m_selectionLabelDirty = true;
			updateSelectedPoint(QSurfaceController3D::invalidSelectionPosition(), 0);
		}
		...
		if (m_selectedSeries) {
			foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
				...
				QVector4D highlightColor =
					QUtils3D::vectorFromColor(cache->series()->singleHighlightColor());
				QSelectionPointer3D *slicePointer = cache->sliceSelectionPointer();
				if (slicePointer) {
				...
				QSelectionPointer3D *mainPointer = cache->mainSelectionPointer();
				if (mainPointer) {
	...
	void QSurfaceRenderer3D::updateSurfaceTextures(QVector<QSurfaceSeries3D *> seriesList)
	{
		foreach(QSurfaceSeries3D *series, seriesList) {
			QSurfaceSeriesRenderCache3D *cache =
				static_cast<QSurfaceSeriesRenderCache3D *>(m_renderCacheList.value(series));
			if (cache) {
	...
	QSeriesRenderCache3D *QSurfaceRenderer3D::createNewCache(QAbstractSeries3D *series)
	{
		m_selectionTexturesDirty = true;
		return new QSurfaceSeriesRenderCache3D(series, this);
	}

	void QSurfaceRenderer3D::cleanCache(QSeriesRenderCache3D *cache)
	{
		QAbstractRenderer3D::cleanCache(cache);
		m_selectionTexturesDirty = true;
	}

	void QSurfaceRenderer3D::updateRows(const QVector<QSurfaceController3D::ChangeRow> &rows)
	{
		foreach(QSurfaceController3D::ChangeRow item, rows) {
			QSurfaceSeriesRenderCache3D *cache =
				static_cast<QSurfaceSeriesRenderCache3D *>(m_renderCacheList.value(item.series));
			QSurfaceDataArray3D &dstArray = cache->dataArray();
			const QRect &sampleSpace = cache->sampleSpace();

			const QSurfaceDataArray3D *srcArray = 0;
			QSurfaceDataProxy3D *dataProxy = item.series->dataProxy();
			if (dataProxy)
	...
	void QSurfaceRenderer3D::updateItems(const QVector<QSurfaceController3D::ChangeItem> &points)
	{
		foreach(QSurfaceController3D::ChangeItem item, points) {
			QSurfaceSeriesRenderCache3D *cache =
				static_cast<QSurfaceSeriesRenderCache3D *>(m_renderCacheList.value(item.series));
			QSurfaceDataArray3D &dstArray = cache->dataArray();
			const QRect &sampleSpace = cache->sampleSpace();
	...
	void QSurfaceRenderer3D::updateSliceDataModel(const QPoint &point)
	{
		foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList)
			static_cast<QSurfaceSeriesRenderCache3D *>(baseCache)->sliceSurfaceObject()->clear();

		if (m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionMultiSeries)) {
			// Find axis coordinates for the selected point
			QSeriesRenderCache3D *selectedCache =
				m_renderCacheList.value(const_cast<QSurfaceSeries3D *>(m_selectedSeries));
			QSurfaceDataArray3D &dataArray =
				static_cast<QSurfaceSeriesRenderCache3D *>(selectedCache)->dataArray();
			QSurfaceDataItem3D item = dataArray.at(point.x())->at(point.y());
			...
			if (m_selectedSeries) {
				QSurfaceSeriesRenderCache3D *cache =
					static_cast<QSurfaceSeriesRenderCache3D *>(
						m_renderCacheList.value(m_selectedSeries));
				if (cache)
					updateSliceObject(static_cast<QSurfaceSeriesRenderCache3D *>(cache), point);
			}
		}
	}

	QPoint QSurfaceRenderer3D::mapCoordsToSampleSpace(QSurfaceSeriesRenderCache3D *cache,
		const QPointF &coords)
	{
		...
		QSurfaceDataItem3D itemBottomLeft = dataArray.at(0)->at(0);
		QSurfaceDataItem3D itemTopRight = dataArray.at(top)->at(right);
		...
		if (itemBottomLeft.x() <= coords.x() && itemTopRight.x() >= coords.x()) {
			...
			QSurfaceDataItem3D item = dataArray.at(0)->at(sampleX);
		...
		if (itemBottomLeft.z() <= coords.y() && itemTopRight.z() >= coords.y()) {
			...
			QSurfaceDataItem3D item = dataArray.at(sampleY)->at(0);
	...
	void QSurfaceRenderer3D::findMatchingRow(float z, int &sample, int direction,
		QSurfaceDataArray3D &dataArray)
	{
		int maxZ = dataArray.size() - 1;
		QSurfaceDataItem3D item = dataArray.at(sample)->at(0);
	...
	void QSurfaceRenderer3D::findMatchingColumn(float x, int &sample, int direction,
		QSurfaceDataArray3D &dataArray)
	{
		int maxX = dataArray.at(0)->size() - 1;
		QSurfaceDataItem3D item = dataArray.at(0)->at(sample);
	...
	void QSurfaceRenderer3D::updateSliceObject(QSurfaceSeriesRenderCache3D *cache, const QPoint &point)
	{
		int column = point.y();
		int row = point.x();

		if ((m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionRow) && row == -1) ||
			(m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionColumn) && column == -1)) {
			cache->sliceSurfaceObject()->clear();
			return;
		}

		QSurfaceDataArray3D &sliceDataArray = cache->sliceDataArray();
		for (int i = 0; i < sliceDataArray.size(); i++)
			delete sliceDataArray.at(i);
		sliceDataArray.clear();
		sliceDataArray.reserve(2);

		QSurfaceDataRow3D *sliceRow;
		QSurfaceDataArray3D &dataArray = cache->dataArray();
		...
		if (m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionRow)) {
			QSurfaceDataRow3D *src = dataArray.at(row);
			sliceRow = new QSurfaceDataRow3D(src->size());
			...
			flipZX = true;
			const QRect &sampleSpace = cache->sampleSpace();
			sliceRow = new QSurfaceDataRow3D(sampleSpace.height());
		...
		QSurfaceDataRow3D *duplicateRow = new QSurfaceDataRow3D(*sliceRow);
		for (int i = 0; i < sliceRow->size(); i++) {
	...
	inline static float getDataValue(const QSurfaceDataArray3D &array, bool searchRow, int index)
	{
		if (searchRow)
	...
	inline static int binarySearchArray(const QSurfaceDataArray3D &array, int maxIdx, float limitValue,
	...
	QRect QSurfaceRenderer3D::calculateSampleRect(const QSurfaceDataArray3D &array)
	{
		QRect sampleSpace;
	...
	void QSurfaceRenderer3D::updateScene(QScene3D *scene)
	{
	...
	void QSurfaceRenderer3D::render(GLuint defaultFboHandle)
	{
		...
		if (m_selectionActive
			&& m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionItem)) {
			for (QSeriesRenderCache3D *baseCache : m_renderCacheList) {
				const QSurfaceSeriesRenderCache3D *cache = static_cast<QSurfaceSeriesRenderCache3D *>(baseCache);
	...
	void QSurfaceRenderer3D::drawSlicedScene()
	{
		if (m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionRow)
		...
		QAxisRenderCache3D &sliceCache = rowMode ? m_axisCacheX : m_axisCacheZ;
		...
		if (!m_renderCacheList.isEmpty()) {
			...
			foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
				...
				if (cache->sliceSurfaceObject()->indexCount() && cache->renderable()) {
					...
					if (cache->surfaceVisible()) {
						QShaderHelper3D *surfaceShader = m_surfaceSliceSmoothShader;
						if (cache->isFlatShadingEnabled())
						...
						if (cache->colorStyle() == QTheme3D::ColorStyleUniform) {
							...
							if (cache->colorStyle() == QTheme3D::ColorStyleObjectGradient) {
			...
			if (drawGrid) {
				...
				m_surfaceGridShader->setUniformValue(m_surfaceGridShader->color(),
					QUtils3D::vectorFromColor(m_cachedTheme->gridLineColor()));
				foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
					QSurfaceSeriesRenderCache3D *cache =
						static_cast<QSurfaceSeriesRenderCache3D *>(baseCache);
		...
		for (int label = 0; label < labelCount; label++) {
			if (m_axisCacheY.labelItems().size() > labelNbr) {
				...
				m_drawer->drawLabel(m_dummyRenderItem, axisLabelItem, viewMatrix, projectionMatrix,
					...
					QDrawer3D::LabelMid, Qt::AlignLeft, true);
			}
			labelNbr++;
		}
		...
		QQuaternion totalRotation = QUtils3D::calculateRotation(rotation);
		...
		for (int label = 0; label < labelCount; label++) {
			if (countLabelItems > labelNbr) {
				...
				QLabelItem3D *axisLabelItem;
				axisLabelItem = sliceCache.labelItems().at(labelNbr);

				m_drawer->drawLabel(m_dummyRenderItem, *axisLabelItem, viewMatrix, projectionMatrix,
					positionComp, totalRotation, 0, QAbstractGraph3D::SelectionRow,
					m_labelShader, m_labelObj, activeCamera,
					false, false, QDrawer3D::LabelBelow,
		...
		QAbstractRenderItem3D *dummyItem(0);
		...
		totalRotation = QUtils3D::calculateRotation(rotation);
	...
	void QSurfaceRenderer3D::drawScene(GLuint defaultFboHandle)
	{
		bool noShadows = true;
		...
		if (!m_isOpenGLES && m_cachedShadowQuality > QAbstractGraph3D::ShadowQualityNone &&
			...
			foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
				QSurfaceSeriesRenderCache3D *cache = static_cast<QSurfaceSeriesRenderCache3D *>(baseCache);
				QSurfaceObject3D *object = cache->surfaceObject();
		...
		if (!m_cachedIsSlicingActivated && (!m_renderCacheList.isEmpty()
			...
			&& m_cachedSelectionMode > QAbstractGraph3D::SelectionNone
			...
			m_surfaceGridShader->bind();
			QAbstractRenderer3D::drawCustomItems(RenderingSelection, m_surfaceGridShader,
				viewMatrix,
		...
		if (m_selectionDirty || m_selectionLabelDirty) {
			QPoint visiblePoint = QSurfaceController3D::invalidSelectionPosition();
			if (m_selectedSeries) {
				QSurfaceSeriesRenderCache3D *cache =
					static_cast<QSurfaceSeriesRenderCache3D *>(
						m_renderCacheList.value(const_cast<QSurfaceSeries3D *>(m_selectedSeries)));
				if (cache && m_selectedPoint != QSurfaceController3D::invalidSelectionPosition()) {
					const QRect &sampleSpace = cache->sampleSpace();
			...
			if (m_cachedSelectionMode == QAbstractGraph3D::SelectionNone
				|| visiblePoint == QSurfaceController3D::invalidSelectionPosition()) {
				m_selectionActive = false;
			}
			else {
				if (m_cachedIsSlicingActivated)
					updateSliceDataModel(visiblePoint);
				if (m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionItem))
		...
		if (!m_renderCacheList.isEmpty()) {
			...
			foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
				...
				if (cache->surfaceObject()->indexCount() && cache->isVisible() &&
					...
					if (cache->surfaceVisible()) {
						QShaderHelper3D *shader = m_surfaceFlatShader;
	...
	void QSurfaceRenderer3D::drawLabels(bool drawSelection, const QCamera3D *activeCamera,
		...
		const QMatrix4x4 &projectionMatrix)
	{
		...
		if (m_axisCacheZ.segmentCount() > 0) {
			...
			for (int label = startIndex; label != endIndex; label = label + indexStep) {
				...
				const QLabelItem3D &axisLabelItem = *m_axisCacheZ.labelItems().at(label);
				// Draw the label here
				if (m_polarGraph) {
		...
		if (m_axisCacheX.segmentCount() > 0) {
			...
			for (int label = startIndex; label != endIndex; label = label + indexStep) {
				...
				const QLabelItem3D &axisLabelItem = *m_axisCacheX.labelItems().at(label);
				if (label == 0 || label == (labelCount - 1)) {
					// If the margin is small, adjust the position of the edge labels to avoid overlapping
					// with labels of the other axes.
					float scaleFactor = m_drawer->scaledFontSize() / axisLabelItem.size().height();
		...
		if (m_axisCacheY.segmentCount() > 0) {
			...
			float offsetValue = 0.0f;
			for (int label = startIndex; label != endIndex; label = label + indexStep) {
				const QLabelItem3D &axisLabelItem = *m_axisCacheY.labelItems().at(label);
				...
				m_drawer->drawLabel(m_dummyRenderItem, axisLabelItem, viewMatrix, projectionMatrix,
					...
					shader, m_labelObj, activeCamera,
					true, true, QDrawer3D::LabelMid, backAlignment, false,
					drawSelection);
				...
				m_drawer->drawLabel(m_dummyRenderItem, axisLabelItem, viewMatrix, projectionMatrix,
					...
					shader, m_labelObj, activeCamera,
					true, true, QDrawer3D::LabelMid, sideAlignment, false,
					drawSelection);
				labelsMaxWidth = qMax(labelsMaxWidth, float(axisLabelItem.size().width()));
			}
	...
	void QSurfaceRenderer3D::updateSelectionMode(QAbstractGraph3D::SelectionFlags mode)
	{
		QAbstractRenderer3D::updateSelectionMode(mode);

		if (m_cachedSelectionMode > QAbstractGraph3D::SelectionNone)
			updateSelectionTextures();
	}

	void QSurfaceRenderer3D::updateSelectionTextures()
	{
	...
	void QSurfaceRenderer3D::createSelectionTexture(QSurfaceSeriesRenderCache3D *cache,
	...
	void QSurfaceRenderer3D::initSelectionBuffer()
	{
	...
	void QSurfaceRenderer3D::fillIdCorner(uchar *p, uchar r, uchar g, uchar b, uchar a)
	{
		p[0] = r;
		p[1] = g;
		p[2] = b;
	...
	void QSurfaceRenderer3D::idToRGBA(uint id, uchar *r, uchar *g, uchar *b, uchar *a)
	{
	...
	void QSurfaceRenderer3D::calculateSceneScalingFactors()
	{
	...
	void QSurfaceRenderer3D::checkFlatSupport(QSurfaceSeriesRenderCache3D *cache)
	{
	...
	void QSurfaceRenderer3D::updateObjects(QSurfaceSeriesRenderCache3D *cache, bool dimensionChanged)
	{
		QSurfaceDataArray3D &dataArray = cache->dataArray();
		const QRect &sampleSpace = cache->sampleSpace();
	...
	void QSurfaceRenderer3D::updateSelectedPoint(const QPoint &position, QSurfaceSeries3D *series)
	{
	...
	void QSurfaceRenderer3D::updateFlipHorizontalGrid(bool flip)
	{
		m_flipHorizontalGrid = flip;
	}

	void QSurfaceRenderer3D::resetClickedStatus()
	{
		m_clickedPosition = QSurfaceController3D::invalidSelectionPosition();
		m_clickedSeries = 0;
	}

	void QSurfaceRenderer3D::loadBackgroundMesh()
	{
		QObjectHelper3D::resetObjectHelper(this, m_backgroundObj,
			QStringLiteral(":/defaultMeshes/background"));
	}

	void QSurfaceRenderer3D::surfacePointSelected(const QPoint &point)
	{
		foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
			QSurfaceSeriesRenderCache3D *cache =
				static_cast<QSurfaceSeriesRenderCache3D *>(baseCache);
		...
		if (m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionMultiSeries)) {
			// Find axis coordinates for the selected point
			QSurfaceSeriesRenderCache3D *selectedCache =
				static_cast<QSurfaceSeriesRenderCache3D *>(
					m_renderCacheList.value(const_cast<QSurfaceSeries3D *>(m_selectedSeries)));
			QSurfaceDataArray3D &dataArray = selectedCache->dataArray();
			QSurfaceDataItem3D item = dataArray.at(point.x())->at(point.y());
			QPointF coords(item.x(), item.z());
	...
	void QSurfaceRenderer3D::updateSelectionPoint(QSurfaceSeriesRenderCache3D *cache, const QPoint &point,
		bool label)
	{
		int row = point.x();
		int column = point.y();
		...
		if (!slicePointer && m_cachedIsSlicingActivated) {
			slicePointer = new QSelectionPointer3D(m_drawer);
			cache->setSliceSelectionPointer(slicePointer);
		}
		QSelectionPointer3D *mainPointer = cache->mainSelectionPointer();
		if (!mainPointer) {
			mainPointer = new QSelectionPointer3D(m_drawer);
		...
		if (m_cachedIsSlicingActivated) {
			...
			else if (m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionColumn)) {
	...
	QPoint QSurfaceRenderer3D::selectionIdToSurfacePoint(uint id)
	{
		...
		QSurfaceSeriesRenderCache3D *selectedCache = 0;
		foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
			QSurfaceSeriesRenderCache3D *cache = static_cast<QSurfaceSeriesRenderCache3D *>(baseCache);
	...
	void QSurfaceRenderer3D::updateShadowQuality(QAbstractGraph3D::ShadowQuality quality)
	{
	...
	void QSurfaceRenderer3D::updateTextures()
	{
	...
	void QSurfaceRenderer3D::updateSlicingActive(bool isSlicing)
	{
	...
	void QSurfaceRenderer3D::initShaders(const QString &vertexShader, const QString &fragmentShader)
	{
		...
		if (!m_isOpenGLES) {
			if (m_cachedShadowQuality > QAbstractGraph3D::ShadowQualityNone) {
				m_surfaceSmoothShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexShadow"),
					QStringLiteral(":/shaders/fragmentSurfaceShadowNoTex"));
				m_surfaceTexturedSmoothShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexShadow"),
					QStringLiteral(":/shaders/fragmentTexturedSurfaceShadow"));
			}
			else {
				m_surfaceSmoothShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertex"),
					QStringLiteral(":/shaders/fragmentSurface"));
				m_surfaceTexturedSmoothShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexTexture"),
					QStringLiteral(":/shaders/fragmentTexture"));
			}
			m_surfaceSliceSmoothShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertex"),
				QStringLiteral(":/shaders/fragmentSurface"));
			if (m_flatSupported) {
				if (m_cachedShadowQuality > QAbstractGraph3D::ShadowQualityNone) {
					m_surfaceFlatShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexSurfaceShadowFlat"),
						QStringLiteral(":/shaders/fragmentSurfaceShadowFlat"));
					m_surfaceTexturedFlatShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexSurfaceShadowFlat"),
						QStringLiteral(":/shaders/fragmentTexturedSurfaceShadowFlat"));
				}
				else {
					m_surfaceFlatShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexSurfaceFlat"),
						QStringLiteral(":/shaders/fragmentSurfaceFlat"));
					m_surfaceTexturedFlatShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexSurfaceFlat"),
						QStringLiteral(":/shaders/fragmentSurfaceTexturedFlat"));
				}
				m_surfaceSliceFlatShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexSurfaceFlat"),
					QStringLiteral(":/shaders/fragmentSurfaceFlat"));
			}
			else {
				m_surfaceFlatShader = 0;
			...
			m_surfaceFlatShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertex"),
				QStringLiteral(":/shaders/fragmentSurfaceES2"));
			m_surfaceTexturedSmoothShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexTexture"),
				QStringLiteral(":/shaders/fragmentTextureES2"));
			m_surfaceTexturedFlatShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexTexture"),
				QStringLiteral(":/shaders/fragmentTextureES2"));
			m_surfaceSliceSmoothShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertex"),
				QStringLiteral(":/shaders/fragmentSurfaceES2"));
			m_surfaceSliceFlatShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertex"),
				QStringLiteral(":/shaders/fragmentSurfaceES2"));
		}
	...
	void QSurfaceRenderer3D::initBackgroundShaders(const QString &vertexShader,
	...
	void QSurfaceRenderer3D::initSelectionShaders()
	{
		if (m_selectionShader)
			delete m_selectionShader;
		m_selectionShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexLabel"),
			QStringLiteral(":/shaders/fragmentLabel"));
		m_selectionShader->initialize();
	}

	void QSurfaceRenderer3D::initSurfaceShaders()
	{
		// Gridline shader
		if (m_surfaceGridShader)
			delete m_surfaceGridShader;
		m_surfaceGridShader = new QShaderHelper3D(this, QStringLiteral(":/shaders/vertexPlainColor"),
	...
	void QSurfaceRenderer3D::initDepthShader()
	{
		if (!m_isOpenGLES) {
			delete m_depthShader;
	...
	void QSurfaceRenderer3D::updateDepthBuffer()
	{
		if (!m_isOpenGLES) {
			m_textureHelper->deleteTexture(&m_depthTexture);
	...
	QVector3D QSurfaceRenderer3D::convertPositionToTranslation(const QVector3D &position,
		bool isAbsolute)
	{
		float xTrans = 0.0f;
		float yTrans = 0.0f;
	...
	void QSurfaceRenderer3D::updateAxisLabels(QAbstractAxis3D::AxisOrientation orientation,
		const QStringList &labels)
	{
		QAbstractRenderer3D::updateAxisLabels(orientation, labels);

		// Angular axis label dimensions affect the chart dimensions
		if (m_polarGraph && orientation == QAbstractAxis3D::AxisOrientationX)
			calculateSceneScalingFactors();
	}

	void QSurfaceRenderer3D::updateAxisTitleVisibility(QAbstractAxis3D::AxisOrientation orientation, bool visible)
	{
		QAbstractRenderer3D::updateAxisTitleVisibility(orientation, visible);
	...
	void QSurfaceRenderer3D::updateMargin(float margin)
	{
