...
namespace QtStackedBar3DVis
{
	...
	QAbstractRenderer3D::QAbstractRenderer3D(QAbstractController3D *controller)
		: QObject(0),
		m_hasNegativeValues(false),
		m_cachedTheme(new QTheme3D()),
		m_drawer(new QDrawer3D(m_cachedTheme)),
		m_cachedShadowQuality(QAbstractGraph3D::ShadowQualityMedium),
		m_autoScaleAdjustment(1.0f),
		m_cachedSelectionMode(QAbstractGraph3D::SelectionNone),
		m_cachedOptimizationHint(QAbstractGraph3D::OptimizationDefault),
		m_textureHelper(0),
		m_depthTexture(0),
		m_cachedScene(new QScene3D()),
...

	{
		...
		m_isOpenGLES = QUtils3D::isOpenGLES();
	#if !defined(QT_OPENGL_ES_2)
		if (!m_isOpenGLES) {
	...
	#endif
		QObject::connect(m_drawer, &QDrawer3D::drawerChanged, this, &QAbstractRenderer3D::updateTextures);
		QObject::connect(this, &QAbstractRenderer3D::needRender, controller,
			&QAbstractController3D::needRender, Qt::QueuedConnection);
		QObject::connect(this, &QAbstractRenderer3D::requestShadowQuality, controller,
			&QAbstractController3D::handleRequestShadowQuality, Qt::QueuedConnection);
	}

	QAbstractRenderer3D::~QAbstractRenderer3D()
	{
		delete m_drawer;
		...
		foreach(QCustomRenderItem3D *item, m_customRenderCache) {
		...
		QObjectHelper3D::releaseObjectHelper(this, m_backgroundObj);
		QObjectHelper3D::releaseObjectHelper(this, m_gridLineObj);
		QObjectHelper3D::releaseObjectHelper(this, m_labelObj);
		QObjectHelper3D::releaseObjectHelper(this, m_positionMapperObj);
	...
	void QAbstractRenderer3D::initializeOpenGL()
	{
		...
		axisCacheForOrientation(QAbstractAxis3D::AxisOrientationX).setDrawer(m_drawer);
		axisCacheForOrientation(QAbstractAxis3D::AxisOrientationY).setDrawer(m_drawer);
		axisCacheForOrientation(QAbstractAxis3D::AxisOrientationZ).setDrawer(m_drawer);
	...
	void QAbstractRenderer3D::render(const GLuint defaultFboHandle)
	{
		if (defaultFboHandle) {
			glDepthMask(true);
			glEnable(GL_DEPTH_TEST);
		...
		QVector4D clearColor = QUtils3D::vectorFromColor(m_cachedTheme->windowColor());
		glClearColor(clearColor.x(), clearColor.y(), clearColor.z(), 1.0f);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
		glDisable(GL_SCISSOR_TEST);
	}

	void QAbstractRenderer3D::updateSelectionState(SelectionState state)
	{
		m_selectionState = state;
	}

	void QAbstractRenderer3D::initGradientShaders(const QString &vertexShader,
	...
	void QAbstractRenderer3D::initStaticSelectedItemShaders(const QString &vertexShader,
		const QString &fragmentShader,
	...
	void QAbstractRenderer3D::initCustomItemShaders(const QString &vertexShader,
		const QString &fragmentShader)
	{
		delete m_customItemShader;
		m_customItemShader = new QShaderHelper3D(this, vertexShader, fragmentShader);
		m_customItemShader->initialize();
	}

	void QAbstractRenderer3D::initVolumeTextureShaders(const QString &vertexShader,
		const QString &fragmentShader,
		...
		const QString &sliceFrameShader)
	{
		...
		delete m_volumeTextureShader;
		m_volumeTextureShader = new QShaderHelper3D(this, vertexShader, fragmentShader);
		m_volumeTextureShader->initialize();

		delete m_volumeTextureLowDefShader;
		m_volumeTextureLowDefShader = new QShaderHelper3D(this, vertexShader, fragmentLowDefShader);
		m_volumeTextureLowDefShader->initialize();

		delete m_volumeTextureSliceShader;
		m_volumeTextureSliceShader = new QShaderHelper3D(this, vertexShader, sliceShader);
		m_volumeTextureSliceShader->initialize();

		delete m_volumeSliceFrameShader;
		m_volumeSliceFrameShader = new QShaderHelper3D(this, sliceFrameVertexShader, sliceFrameShader);
		m_volumeSliceFrameShader->initialize();
	}

	void QAbstractRenderer3D::initLabelShaders(const QString &vertexShader, const QString &fragmentShader)
	{
		delete m_labelShader;
		m_labelShader = new QShaderHelper3D(this, vertexShader, fragmentShader);
		m_labelShader->initialize();
	}

	void QAbstractRenderer3D::initCursorPositionShaders(const QString &vertexShader,
		const QString &fragmentShader)
	{
		// Init the shader
		delete m_cursorPositionShader;
		m_cursorPositionShader = new QShaderHelper3D(this, vertexShader, fragmentShader);
		m_cursorPositionShader->initialize();
	}

	void QAbstractRenderer3D::initCursorPositionBuffer()
	{
		m_textureHelper->deleteTexture(&m_cursorPositionTexture);
		m_textureHelper->glDeleteFramebuffers(1, &m_cursorPositionFrameBuffer);
		m_cursorPositionFrameBuffer = 0;
	...
	void QAbstractRenderer3D::updateTheme(QTheme3D *theme)
	{
	...
	void QAbstractRenderer3D::updateScene(QScene3D *scene)
	{
		...
		if (QScene3D::invalidSelectionPoint() == logicalPixelPosition) {
		...
		if (QScene3D::invalidSelectionPoint() != logicalGraphPosition)
	...
	void QAbstractRenderer3D::updateTextures()
	{
	...
	void QAbstractRenderer3D::reInitShaders()
	{
		if (!m_isOpenGLES) {
			if (m_cachedShadowQuality > QAbstractGraph3D::ShadowQualityNone) {
				if (m_cachedOptimizationHint.testFlag(QAbstractGraph3D::OptimizationStatic)
					&& qobject_cast<QScatterRenderer3D *>(this)) {
	...
	void QAbstractRenderer3D::handleShadowQualityChange()
	{
		reInitShaders();
		...
		if (m_isOpenGLES && m_cachedShadowQuality != QAbstractGraph3D::ShadowQualityNone) {
			emit requestShadowQuality(QAbstractGraph3D::ShadowQualityNone);
			qWarning("Shadows are not yet supported for OpenGL ES2");
			m_cachedShadowQuality = QAbstractGraph3D::ShadowQualityNone;
		}
	}

	void QAbstractRenderer3D::updateSelectionMode(QAbstractGraph3D::SelectionFlags mode)
	{
		m_cachedSelectionMode = mode;
		m_selectionDirty = true;
	}

	void QAbstractRenderer3D::updateAspectRatio(float ratio)
	{
	...
	void QAbstractRenderer3D::updateHorizontalAspectRatio(float ratio)
	{
	...
	void QAbstractRenderer3D::updatePolar(bool enable)
	{
		m_polarGraph = enable;
		foreach(QSeriesRenderCache3D *cache, m_renderCacheList)
			cache->setDataDirty(true);
	...
	void QAbstractRenderer3D::updateRadialLabelOffset(float offset)
	{
		m_radialLabelOffset = offset;
	}

	void QAbstractRenderer3D::updateMargin(float margin)
	{
		m_requestedMargin = margin;
	}

	void QAbstractRenderer3D::updateOptimizationHint(QAbstractGraph3D::OptimizationHints hint)
	{
	...
	void QAbstractRenderer3D::handleResize()
	{
	...
	void QAbstractRenderer3D::calculateZoomLevel()
	{
	...
	void QAbstractRenderer3D::updateAxisType(QAbstractAxis3D::AxisOrientation orientation,
		QAbstractAxis3D::AxisType type)
	{
		axisCacheForOrientation(orientation).setType(type);
	}

	void QAbstractRenderer3D::updateAxisTitle(QAbstractAxis3D::AxisOrientation orientation,
		const QString &title)
	{
		axisCacheForOrientation(orientation).setTitle(title);
	}

	void QAbstractRenderer3D::updateAxisLabels(QAbstractAxis3D::AxisOrientation orientation,
		const QStringList &labels)
	{
		axisCacheForOrientation(orientation).setLabels(labels);
	}

	void QAbstractRenderer3D::updateAxisRange(QAbstractAxis3D::AxisOrientation orientation,
		float min, float max)
	{
		QAxisRenderCache3D &cache = axisCacheForOrientation(orientation);
		cache.setMin(min);
		cache.setMax(max);
	...
	void QAbstractRenderer3D::updateAxisSegmentCount(QAbstractAxis3D::AxisOrientation orientation,
		int count)
	{
		QAxisRenderCache3D &cache = axisCacheForOrientation(orientation);
	...
	void QAbstractRenderer3D::updateAxisSubSegmentCount(QAbstractAxis3D::AxisOrientation orientation,
		int count)
	{
		QAxisRenderCache3D &cache = axisCacheForOrientation(orientation);
	...
	void QAbstractRenderer3D::updateAxisLabelFormat(QAbstractAxis3D::AxisOrientation orientation,
		const QString &format)
	{
		axisCacheForOrientation(orientation).setLabelFormat(format);
	}

	void QAbstractRenderer3D::updateAxisReversed(QAbstractAxis3D::AxisOrientation orientation,
		bool enable)
	{
	...
	void QAbstractRenderer3D::updateAxisFormatter(QAbstractAxis3D::AxisOrientation orientation,
	...
	void QAbstractRenderer3D::updateAxisLabelAutoRotation(QAbstractAxis3D::AxisOrientation orientation,
		float angle)
	{
		QAxisRenderCache3D &cache = axisCacheForOrientation(orientation);
	...
	void QAbstractRenderer3D::updateAxisTitleVisibility(QAbstractAxis3D::AxisOrientation orientation,
		bool visible)
	{
		QAxisRenderCache3D &cache = axisCacheForOrientation(orientation);
	...
	void QAbstractRenderer3D::updateAxisTitleFixed(QAbstractAxis3D::AxisOrientation orientation,
		bool fixed)
	{
		QAxisRenderCache3D &cache = axisCacheForOrientation(orientation);
	...
	void QAbstractRenderer3D::modifiedSeriesList(const QVector<QAbstractSeries3D *> &seriesList)
	{
		foreach(QAbstractSeries3D *series, seriesList) {
			QSeriesRenderCache3D *cache = m_renderCacheList.value(series, 0);
			if (cache)
				cache->setDataDirty(true);
		}
	}

	void QAbstractRenderer3D::fixMeshFileName(QString &fileName, QAbstractSeries3D::Mesh mesh)
	{
	...
	void QAbstractRenderer3D::updateSeries(const QList<QAbstractSeries3D *> &seriesList)
	{
		foreach(QSeriesRenderCache3D *cache, m_renderCacheList)
		...
		for (int i = 0; i < seriesCount; i++) {
			QAbstractSeries3D *series = seriesList.at(i);
			QSeriesRenderCache3D *cache = m_renderCacheList.value(series);
	...
	void QAbstractRenderer3D::updateCustomData(const QList<QCustomItem3D *> &customItems)
	{
		...
		for (int i = 0; i < itemCount; i++) {
			QCustomItem3D *item = customItems.at(i);
			QCustomRenderItem3D *renderItem = m_customRenderCache.value(item);
			if (!renderItem)
		...
		foreach(QCustomRenderItem3D *renderItem, m_customRenderCache) {
	...
	void QAbstractRenderer3D::updateCustomItems()
	{
		// Check all items
		foreach(QCustomRenderItem3D *item, m_customRenderCache)
	...
	QSeriesRenderCache3D *QAbstractRenderer3D::createNewCache(QAbstractSeries3D *series)
	{
		return new QSeriesRenderCache3D(series, this);
	}

	void QAbstractRenderer3D::cleanCache(QSeriesRenderCache3D *cache)
	{
	...
	QAxisRenderCache3D &QAbstractRenderer3D::axisCacheForOrientation(
		QAbstractAxis3D::AxisOrientation orientation)
	{
		...
		case QAbstractAxis3D::AxisOrientationZ:
			return m_axisCacheZ;
		default:
			qFatal("QAbstractRenderer3D::axisCacheForOrientation");
			return m_axisCacheX;
		}
	}

	void QAbstractRenderer3D::lowerShadowQuality()
	{
		QAbstractGraph3D::ShadowQuality newQuality = QAbstractGraph3D::ShadowQualityNone;

		switch (m_cachedShadowQuality) {
		case QAbstractGraph3D::ShadowQualityHigh:
			qWarning("Creating high quality shadows failed. Changing to medium quality.");
			newQuality = QAbstractGraph3D::ShadowQualityMedium;
			break;
		case QAbstractGraph3D::ShadowQualityMedium:
			qWarning("Creating medium quality shadows failed. Changing to low quality.");
			newQuality = QAbstractGraph3D::ShadowQualityLow;
			break;
		case QAbstractGraph3D::ShadowQualityLow:
			qWarning("Creating low quality shadows failed. Switching shadows off.");
			newQuality = QAbstractGraph3D::ShadowQualityNone;
			break;
		case QAbstractGraph3D::ShadowQualitySoftHigh:
			...
			newQuality = QAbstractGraph3D::ShadowQualitySoftMedium;
			break;
		case QAbstractGraph3D::ShadowQualitySoftMedium:
			...
			newQuality = QAbstractGraph3D::ShadowQualitySoftLow;
			break;
	...
	void QAbstractRenderer3D::drawAxisTitleY(const QVector3D &sideLabelRotation,
		...
		QAbstractRenderItem3D &dummyItem,
		const QCamera3D *activeCamera,
		...
		QShaderHelper3D *shader)
	{
		...
		m_drawer->drawLabel(dummyItem, m_axisCacheY.titleItem(), viewMatrix,
			...
			true, true, QDrawer3D::LabelMid, Qt::AlignBottom);
	}

	void QAbstractRenderer3D::drawAxisTitleX(const QVector3D &labelRotation,
	...
	void QAbstractRenderer3D::drawAxisTitleZ(const QVector3D &labelRotation,
		const QVector3D &labelTrans,
		const QQuaternion &totalRotation,
		QAbstractRenderItem3D &dummyItem,
		const QCamera3D *activeCamera,
	...
	void QAbstractRenderer3D::loadGridLineMesh()
	{
		QObjectHelper3D::resetObjectHelper(this, m_gridLineObj,
			QStringLiteral(":/defaultMeshes/plane"));
	}

	void QAbstractRenderer3D::loadLabelMesh()
	{
		QObjectHelper3D::resetObjectHelper(this, m_labelObj,
			QStringLiteral(":/defaultMeshes/plane"));
	}

	void QAbstractRenderer3D::loadPositionMapperMesh()
	{
		QObjectHelper3D::resetObjectHelper(this, m_positionMapperObj,
			QStringLiteral(":/defaultMeshes/barFull"));
	}

	void QAbstractRenderer3D::generateBaseColorTexture(const QColor &color, GLuint *texture)
	{
		m_textureHelper->deleteTexture(texture);
		*texture = m_textureHelper->createUniformTexture(color);
	}

	void QAbstractRenderer3D::fixGradientAndGenerateTexture(QLinearGradient *gradient,
	...
	QLabelItem3D &QAbstractRenderer3D::selectionLabelItem()
	{
		if (!m_selectionLabelItem)
			m_selectionLabelItem = new QLabelItem3D;
		return *m_selectionLabelItem;
	}

	void QAbstractRenderer3D::setSelectionLabel(const QString &label)
	{
		if (m_selectionLabelItem)
	...
	QString &QAbstractRenderer3D::selectionLabel()
	{
		return m_selectionLabel;
	}

	QVector4D QAbstractRenderer3D::indexToSelectionColor(GLint index)
	{
	...
	QCustomRenderItem3D *QAbstractRenderer3D::addCustomItem(QCustomItem3D *item)
	{
		QCustomRenderItem3D *newItem = new QCustomRenderItem3D();
		...
		if (item->d_ptr->m_isLabelItem) {
			QCustomLabel3D *labelItem = static_cast<QCustomLabel3D *>(item);
			...
			QCustomVolume3D *volumeItem = static_cast<QCustomVolume3D *>(item);
	...
	void QAbstractRenderer3D::recalculateCustomItemScalingAndPos(QCustomRenderItem3D *item)
	{
	...
	void QAbstractRenderer3D::updateCustomItem(QCustomRenderItem3D *renderItem)
	{
		QCustomItem3D *item = renderItem->itemPointer();
	...
	void QAbstractRenderer3D::updateCustomItemPositions()
	{
		foreach(QCustomRenderItem3D *renderItem, m_customRenderCache)
			recalculateCustomItemScalingAndPos(renderItem);
	}

	void QAbstractRenderer3D::drawCustomItems(RenderingState state,
		QShaderHelper3D *regularShader,
		const QMatrix4x4 &viewMatrix,
		...
		GLfloat reflection)
	{
		...
		QShaderHelper3D *shader = regularShader;
		shader->bind();
		...
		while (loopCount < 2) {
			foreach(QCustomRenderItem3D *item, m_customRenderCache) {
				...
				if (RenderingNormal == state) {
					// Normal render
					QShaderHelper3D *prevShader = shader;
	...
	void QAbstractRenderer3D::drawVolumeSliceFrame(const QCustomRenderItem3D *item, Qt::Axis axis,
		const QMatrix4x4 &projectionViewMatrix)
	{
	...
	void QAbstractRenderer3D::queriedGraphPosition(const QMatrix4x4 &projectionViewMatrix,
		...
		GLuint defaultFboHandle)
	{
		...
		QVector4D dataColor = QUtils3D::getSelection(m_graphPositionQuery,
			m_primarySubViewport.height());
		if (dataColor.w() > 0.0f) {
	...
	void QAbstractRenderer3D::fixContextBeforeDelete()
	{
	...
	void QAbstractRenderer3D::restoreContextAfterDelete()
	{
	...
	void QAbstractRenderer3D::calculatePolarXZ(const QVector3D &dataPos, float &x, float &z) const
	{
	...
	void QAbstractRenderer3D::drawRadialGrid(QShaderHelper3D *shader, float yFloorLinePos,
		const QMatrix4x4 &projectionViewMatrix,
	...
	void QAbstractRenderer3D::drawAngularGrid(QShaderHelper3D *shader, float yFloorLinePos,
		const QMatrix4x4 &projectionViewMatrix,
		const QMatrix4x4 &depthMatrix)
	{
	...
	float QAbstractRenderer3D::calculatePolarBackgroundMargin()
	{
	...
	void QAbstractRenderer3D::updateCameraViewport()
	{
