...
namespace QtStackedBar3DVis
{
	...
	QScatterObjectBufferHelper3D::QScatterObjectBufferHelper3D()
		: m_scaleY(0.0f)
	{
	}
	...
	void QScatterObjectBufferHelper3D::fullLoad(QScatterSeriesRenderCache3D *cache, qreal dotScale)
	{
		...
		else if (cache->colorStyle() == QTheme3D::ColorStyleObjectGradient)
		...
		for (uint i = 0; i < renderArraySize; i++) {
			const QScatterRenderItem3D &item = renderArray.at(i);
			if (!item.isVisible())
				continue;
			else
	...
	void QScatterObjectBufferHelper3D::updateUVs(QScatterSeriesRenderCache3D *cache)
	{
		QObjectHelper3D *dotObj = cache->object();
	...
	uint QScatterObjectBufferHelper3D::createRangeGradientUVs(QScatterSeriesRenderCache3D *cache,
		QVector<QVector2D> &buffered_uvs)
	{
		QObjectHelper3D *dotObj = cache->object();
		const int uvsCount = dotObj->indexedUVs().count();
		...
		for (int i = 0; i < updateSize; i++) {
			...
			const QScatterRenderItem3D &item = renderArray.at(index);
			if (!item.isVisible())
				continue;
	...
	uint QScatterObjectBufferHelper3D::createObjectGradientUVs(QScatterSeriesRenderCache3D *cache,
		QVector<QVector2D> &buffered_uvs,
	...
	void QScatterObjectBufferHelper3D::update(QScatterSeriesRenderCache3D *cache, qreal dotScale)
	{
		QObjectHelper3D *dotObj = cache->object();
		const QScatterRenderItemArray3D &renderArray = cache->renderArray();
		const bool updateAll = (cache->updateIndices().size() == 0);
