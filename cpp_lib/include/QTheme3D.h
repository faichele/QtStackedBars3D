...
namespace QtStackedBar3DVis
{
	...
	class QTheme3DPrivate;

	class QT_STACKEDBAR3D_EXPORT QTheme3D : public QObject
	{
		Q_OBJECT
			Q_ENUMS(ColorStyle)
			Q_ENUMS(Theme)
	...
	public:
		explicit QTheme3D(QObject *parent = Q_NULLPTR);
		explicit QTheme3D(Theme themeType, QObject *parent = Q_NULLPTR);
		virtual ~QTheme3D();
	...
	Q_SIGNALS:
		void typeChanged(QTheme3D::Theme themeType);
	...
	protected:
		explicit QTheme3D(QTheme3DPrivate *d, Theme themeType, QObject *parent = Q_NULLPTR);

		QScopedPointer<QTheme3DPrivate> d_ptr;

	private:
		Q_DISABLE_COPY(QTheme3D)

		friend class QThemeManager3D;
