...
#ifndef Q_BARS_RENDERER_3D_H
#define Q_BARS_RENDERER_3D_H
...
namespace QtStackedBar3DVis
{
	...
	class QLabelItem3D;
	class QScene3D;
	class QBarSeriesRenderCache3D;

	class QT_STACKEDBAR3D_EXPORT QBarsRenderer3D : public QAbstractRenderer3D
	{
		Q_OBJECT
	...
	private:
		...
		// Internal state
		QBarRenderItem3D *m_selectedBar; // points to renderitem array
		...
		QBarSeriesRenderCache3D *m_selectedSeriesCache;
		QBarRenderItem3D m_dummyBarRenderItem;
	...
	public:
		explicit QBarsRenderer3D(QBarsController3D *controller);
		~QBarsRenderer3D();

		void updateData();
		void updateSeries(const QList<QAbstractSeries3D *> &seriesList);
		QSeriesRenderCache3D *createNewCache(QAbstractSeries3D *series);
		void updateRows(const QVector<QBarsController3D::ChangeRow> &rows);
		void updateItems(const QVector<QBarsController3D::ChangeItem> &items);
	...
	protected:
		...
		void updateSelectedBar(const QPoint &position, QBarSeries3D *series);
	...
	private:
		...
		bool drawBars(QBarRenderItem3D **selectedBar, const QMatrix4x4 &depthProjectionViewMatrix,
		...
		QBarSeries3D *selectionColorToSeries(const QVector4D &selectionColor);

		inline void updateRenderRow(const QBarDataRow3D *dataRow, BarRenderItemRow3D &renderRow);
		inline void updateRenderItem(const QBarDataItem3D &dataItem, QBarRenderItem3D &renderItem);
