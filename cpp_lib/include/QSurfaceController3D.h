...
namespace QtStackedBar3DVis
{
	...
	class QT_STACKEDBAR3D_EXPORT QSurfaceController3D : public QAbstractController3D
	{
		Q_OBJECT
	...
	private:
		...
		QSurfaceSeries3D *m_selectedSeries; // Points to the series for which the point is selected in
		...
		QVector<QSurfaceSeries3D *> m_changedTextures;

	public:
		explicit QSurfaceController3D(QRect rect, QScene3D *scene = 0);
		~QSurfaceController3D();

		virtual void initializeOpenGL();
		virtual void synchDataToRenderer();

		void setSelectionMode(QAbstractGraph3D::SelectionFlags mode);
		void setSelectedPoint(const QPoint &position, QSurfaceSeries3D *series, bool enterSlice);
		virtual void clearSelection();

		inline QSurfaceSeries3D *selectedSeries() const { return m_selectedSeries; }
		...
		virtual QList<QSurfaceSeries3D *> surfaceSeriesList();

		void setFlipHorizontalGrid(bool flip);
		bool flipHorizontalGrid() const;

		void updateSurfaceTexture(QSurfaceSeries3D *series);
	...
	Q_SIGNALS:
		void selectedSeriesChanged(QSurfaceSeries3D *series);
		void flipHorizontalGridChanged(bool flip);

	private:
		Q_DISABLE_COPY(QSurfaceController3D)
	};
