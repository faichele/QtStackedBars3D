...
namespace QtStackedBar3DVis
{
	...
	class QBars3DPrivate;

	class QT_STACKEDBAR3D_EXPORT QBars3D : public QAbstractGraph3D
	{
		Q_OBJECT
			...
			Q_PROPERTY(QBarSeries3D *primarySeries READ primarySeries WRITE setPrimarySeries NOTIFY primarySeriesChanged)
			Q_PROPERTY(QBarSeries3D *selectedSeries READ selectedSeries NOTIFY selectedSeriesChanged)
			Q_PROPERTY(float floorLevel READ floorLevel WRITE setFloorLevel NOTIFY floorLevelChanged)

	public:
		explicit QBars3D(const QSurfaceFormat *format = Q_NULLPTR, QWindow *parent = Q_NULLPTR);
		virtual ~QBars3D();

		void setPrimarySeries(QBarSeries3D *series);
		QBarSeries3D *primarySeries() const;
		void addSeries(QBarSeries3D *series);
		void removeSeries(QBarSeries3D *series);
		void insertSeries(int index, QBarSeries3D *series);
		QList<QBarSeries3D *> seriesList() const;
		...
		QBarSeries3D *selectedSeries() const;
	...
	private:
		QBars3DPrivate *dptr();
		const QBars3DPrivate *dptrc() const;
		Q_DISABLE_COPY(QBars3D)
	};
