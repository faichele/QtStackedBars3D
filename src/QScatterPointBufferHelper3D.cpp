...
namespace QtStackedBar3DVis
{
	...
	QScatterPointBufferHelper3D::QScatterPointBufferHelper3D()
		: m_pointbuffer(0),
		m_oldRemoveIndex(-1)
	{
	}
	...
	GLuint QScatterPointBufferHelper3D::pointBuf()
	{
		if (!m_meshDataLoaded)
	...
	void QScatterPointBufferHelper3D::pushPoint(uint pointIndex)
	{
	...
	void QScatterPointBufferHelper3D::popPoint()
	{
		if (m_oldRemoveIndex >= 0) {
			glBindBuffer(GL_ARRAY_BUFFER, m_pointbuffer);
			glBufferSubData(GL_ARRAY_BUFFER, m_oldRemoveIndex * sizeof(QVector3D),
	...
	void QScatterPointBufferHelper3D::load(QScatterSeriesRenderCache3D *cache)
	{
	...
	void QScatterPointBufferHelper3D::update(QScatterSeriesRenderCache3D *cache)
	{
	...
	void QScatterPointBufferHelper3D::updateUVs(QScatterSeriesRenderCache3D *cache)
	{
		// It may be that the buffer hasn't yet been initialized, in case the entire series was
		// hidden items. No need to update in that case.
		if (m_indexCount > 0) {
	...
	void QScatterPointBufferHelper3D::createRangeGradientUVs(QScatterSeriesRenderCache3D *cache,
		QVector<QVector2D> &buffered_uvs)
	{
		const QScatterRenderItemArray3D &renderArray = cache->renderArray();
