...
namespace QtStackedBar3DVis
{
	...
	* \brief QScene3D class provides description of the 3D scene being visualized.
	...
	* by default resized by the \a QScene3D. To override the resize behavior you need to listen to both
	...
	* \l{QAbstractGraph3D::SelectionRow}{AbstractGraph3D.SelectionRow} or
	* \l{QAbstractGraph3D::SelectionColumn}{AbstractGraph3D.SelectionColumn}
	...
	*/
	QScene3D::QScene3D(QObject *parent) :
		QObject(parent),
		d_ptr(new QScene3DPrivate(this))
	{
		setActiveCamera(new QCamera3D(0));
		setActiveLight(new QLight3D(0));
	}
	...
	*/
	QRect QScene3D::primarySubViewport() const
	{
	...
	void QScene3D::setPrimarySubViewport(const QRect &primarySubViewport)
	{
	...
	*/
	bool QScene3D::isPointInPrimarySubView(const QPoint &point)
	{
		int x = point.x();
		int y = point.y();
	...
	*/
	bool QScene3D::isPointInSecondarySubView(const QPoint &point)
	{
		int x = point.x();
		int y = point.y();
	...
	/*!
	* \property QScene3D::secondarySubViewport
	*
	...
	* viewport, the viewport is resized accordingly.
	*/
	QRect QScene3D::secondarySubViewport() const
	{
	...
	void QScene3D::setSecondarySubViewport(const QRect &secondarySubViewport)
	{
	...
	/*!
	* \property QScene3D::selectionQueryPosition
	*
	...
	* invalidSelectionPoint().
	*
	* \sa QAbstractGraph3D::selectedElement
	*/
	void QScene3D::setSelectionQueryPosition(const QPoint &point)
	{
	...
	QPoint QScene3D::selectionQueryPosition() const
	{
	...
	*/
	QPoint QScene3D::invalidSelectionPoint()
	{
	...
	/*!
	* \property QScene3D::graphPositionQuery
	*
	* \brief The coordinates for the user input that should be processed
	...
	* After the rendering pass, this property is returned to its default state of
	* invalidSelectionPoint(). The queried graph position can be read from the
	* QAbstractGraph3D::queriedGraphPosition property after the next render pass.
	*
	* There is no single correct 3D coordinate to match a particular screen position, so to be
	* consistent, the queries are always done against the inner sides of an invisible box surrounding
	* the graph.
	...
	* \note Bar graphs allow graph position queries only at the graph floor level.
	*
	* \sa QAbstractGraph3D::queriedGraphPosition
	*/
	void QScene3D::setGraphPositionQuery(const QPoint &point)
	{
	...
	QPoint QScene3D::graphPositionQuery() const
	{
		return d_ptr->m_graphPositionQueryPosition;
	}

	/*!
	* \property QScene3D::slicingActive
	*
	* \brief Whether the 2D slicing view is currently active.
	*
	* If \c true, QAbstractGraph3D::selectionMode must have either
	* QAbstractGraph3D::SelectionRow or QAbstractGraph3D::SelectionColumn set
	* to a valid selection.
	* \note Not all visualizations support the 2D slicing view.
	*/
	bool QScene3D::isSlicingActive() const
	{
		return d_ptr->m_isSlicingActive;
	}

	void QScene3D::setSlicingActive(bool isSlicing)
	{
	...
	/*!
	* \property QScene3D::secondarySubviewOnTop
	*
	* \brief Whether the 2D slicing view or the 3D view is drawn on top.
	*/
	bool QScene3D::isSecondarySubviewOnTop() const
	{
		return d_ptr->m_isSecondarySubviewOnTop;
	}

	void QScene3D::setSecondarySubviewOnTop(bool isSecondaryOnTop)
	{
	...
	/*!
	* \property QScene3D::activeCamera
	*
	* \brief The currently active camera in the 3D scene.
	*
	* When a new QCamera3D object is set, it is automatically added as child of
	* the scene.
	*/
	QCamera3D *QScene3D::activeCamera() const
	{
		return d_ptr->m_camera;
	}

	void QScene3D::setActiveCamera(QCamera3D *camera)
	{
		Q_ASSERT(camera);
		...
		if (camera != d_ptr->m_camera) {
			if (d_ptr->m_camera) {
				disconnect(d_ptr->m_camera, &QCamera3D::xRotationChanged, d_ptr.data(),
					&QScene3DPrivate::needRender);
				disconnect(d_ptr->m_camera, &QCamera3D::yRotationChanged, d_ptr.data(),
					&QScene3DPrivate::needRender);
				disconnect(d_ptr->m_camera, &QCamera3D::zoomLevelChanged, d_ptr.data(),
					&QScene3DPrivate::needRender);
			}
			...
			if (camera) {
				connect(camera, &QCamera3D::xRotationChanged, d_ptr.data(),
					&QScene3DPrivate::needRender);
				connect(camera, &QCamera3D::yRotationChanged, d_ptr.data(),
					&QScene3DPrivate::needRender);
				connect(camera, &QCamera3D::zoomLevelChanged, d_ptr.data(),
					&QScene3DPrivate::needRender);
			}
	...
	/*!
	* \property QScene3D::activeLight
	*
	* \brief The currently active light in the 3D scene.
	*
	* When a new QLight3D objects is set, it is automatically added as child of
	* the scene.
	*/
	QLight3D *QScene3D::activeLight() const
	{
		return d_ptr->m_light;
	}

	void QScene3D::setActiveLight(QLight3D *light)
	{
		Q_ASSERT(light);
	...
	/*!
	* \property QScene3D::devicePixelRatio
	*
	* \brief The device pixel ratio that is used when mapping input
	* coordinates to pixel coordinates.
	*/
	float QScene3D::devicePixelRatio() const
	{
		return d_ptr->m_devicePixelRatio;
	}

	void QScene3D::setDevicePixelRatio(float pixelRatio)
	{
	...
	QScene3DPrivate::QScene3DPrivate(QScene3D *q) :
		QObject(0),
		q_ptr(q),
		...
		m_selectionQueryPosition(QScene3D::invalidSelectionPoint()),
		m_graphPositionQueryPosition(QScene3D::invalidSelectionPoint()),
		m_windowSize(QSize(0, 0)),
		m_sceneDirty(true)
	{
	}

	QScene3DPrivate::~QScene3DPrivate()
	{
		delete m_camera;
		delete m_light;
	}
	...
	void QScene3DPrivate::sync(QScene3DPrivate &other)
	{
	...
	void QScene3DPrivate::setViewport(const QRect &viewport)
	{
	...
	void QScene3DPrivate::setViewportSize(int width, int height)
	{
	...
	*/
	void QScene3DPrivate::setWindowSize(const QSize &size)
	{
	...
	QSize QScene3DPrivate::windowSize() const
	{
		return m_windowSize;
	}

	void QScene3DPrivate::calculateSubViewports()
	{
	...
	void QScene3DPrivate::updateGLViewport()
	{
		// Update GL viewport
	...
	void QScene3DPrivate::updateGLSubViewports()
	{
	...
	QRect QScene3DPrivate::glViewport()
	{
		return m_glViewport;
	}

	QRect QScene3DPrivate::glPrimarySubViewport()
	{
		return m_glPrimarySubViewport;
	}

	QRect QScene3DPrivate::glSecondarySubViewport()
	{
	...
	*/
	void QScene3DPrivate::setLightPositionRelativeToCamera(const QVector3D &relativePosition,
	...
	void QScene3DPrivate::markDirty()
	{
		m_sceneDirty = true;
		emit needRender();
	}

	bool QScene3DPrivate::isInArea(const QRect &area, int x, int y) const
	{
		int areaMinX = area.x();
