...
namespace QtStackedBar3DVis
{
	...
	QBarsController3D::QBarsController3D(QRect boundRect, QScene3D *scene)
	...
	QBarsController3D::~QBarsController3D()
	{
	}

	void QBarsController3D::initializeOpenGL()
	{
		...
		m_renderer = new QBarsRenderer3D(this);
	...
	void QBarsController3D::synchDataToRenderer()
	{
		QMutexLocker mutexLocker(&m_renderMutex);
	...
	void QBarsController3D::handleArrayReset()
	{
		QBarSeries3D *series;
		if (qobject_cast<QBarDataProxy3D *>(sender()))
			series = static_cast<QBarDataProxy3D *>(sender())->series();
		else
			series = static_cast<QBarSeries3D *>(sender());
	...
	void QBarsController3D::handleRowsAdded(int startIndex, int count)
	{
		Q_UNUSED(startIndex)
			Q_UNUSED(count)
			QBarSeries3D *series = static_cast<QBarDataProxy3D *>(sender())->series();
		if (series->isVisible()) {
			adjustAxisRanges();
			m_isDataDirty = true;
		}
	...
	void QBarsController3D::handleRowsChanged(int startIndex, int count)
	{
		QBarSeries3D *series = static_cast<QBarDataProxy3D *>(sender())->series();
	...
	void QBarsController3D::handleRowsRemoved(int startIndex, int count)
	{
		Q_UNUSED(startIndex)
			Q_UNUSED(count)
	...
	void QBarsController3D::handleRowsInserted(int startIndex, int count)
	{
		Q_UNUSED(startIndex)
			Q_UNUSED(count)
			QBarSeries3D *series = static_cast<QBarDataProxy3D *>(sender())->series();
	...
	void QBarsController3D::handleItemChanged(int rowIndex, int columnIndex)
	{
		QBarSeries3D *series = static_cast<QBarDataProxy3D *>(sender())->series();
	...
	void QBarsController3D::handleDataRowLabelsChanged()
	{
		if (m_axisZ) {
	...
	void QBarsController3D::handleDataColumnLabelsChanged()
	{
		if (m_axisX) {
			// Grab a sublist equal to data window (no need to have more labels in axis)
			...
			QStringList subList;
			if (m_primarySeries && m_primarySeries->dataProxy()) {
				subList = static_cast<QBarDataProxy3D *>(m_primarySeries->dataProxy())
	...
	void QBarsController3D::handleAxisAutoAdjustRangeChangedInOrientation(
	...
	void QBarsController3D::handleSeriesVisibilityChangedBySender(QObject *sender)
	{
	...
	void QBarsController3D::handlePendingClick()
	{
		...
		QBarSeries3D *series = static_cast<QBarSeries3D *>(m_renderer->clickedSeries());
	...
	QPoint QBarsController3D::invalidSelectionPosition()
	{
		static QPoint invalidSelectionPos(-1, -1);
		return invalidSelectionPos;
	}

	void QBarsController3D::setAxisX(QAbstractAxis3D *axis)
	{
		QAbstractController3D::setAxisX(axis);
		handleDataColumnLabelsChanged();
	}

	void QBarsController3D::setAxisZ(QAbstractAxis3D *axis)
	{
		QAbstractController3D::setAxisZ(axis);
		handleDataRowLabelsChanged();
	}

	void QBarsController3D::setPrimarySeries(QBarSeries3D *series)
	{
		if (!series) {
			if (m_seriesList.size())
				series = static_cast<QBarSeries3D *>(m_seriesList.at(0));
		}
	...
	QBarSeries3D *QBarsController3D::primarySeries() const
	{
		return m_primarySeries;
	}

	void QBarsController3D::addSeries(QAbstractSeries3D *series)
	{
		insertSeries(m_seriesList.size(), series);
	}

	void QBarsController3D::removeSeries(QAbstractSeries3D *series)
	{
		...
		if (series == m_primarySeries) {
			if (m_seriesList.size())
				m_primarySeries = static_cast<QBarSeries3D *>(m_seriesList.at(0));
			else
				m_primarySeries = 0;
	...
	void QBarsController3D::insertSeries(int index, QAbstractSeries3D *series)
	{
	...
	QList<QBarSeries3D *> QBarsController3D::barSeriesList()
	{
		QList<QAbstractSeries3D *> abstractSeriesList = seriesList();
		QList<QBarSeries3D *> barSeriesList;
		foreach(QAbstractSeries3D *abstractSeries, abstractSeriesList) {
			QBarSeries3D *barSeries = qobject_cast<QBarSeries3D *>(abstractSeries);
			if (barSeries)
	...
	void QBarsController3D::handleAxisRangeChangedBySender(QObject *sender)
	{
		// Data window changed
	...
	void QBarsController3D::setMultiSeriesScaling(bool uniform)
	{
	...
	bool QBarsController3D::multiSeriesScaling() const
	{
		return m_isMultiSeriesUniform;
	}

	void QBarsController3D::setBarSpecs(GLfloat thicknessRatio, const QSizeF &spacing, bool relative)
	{
	...
	GLfloat QBarsController3D::barThickness()
	{
		return m_barThicknessRatio;
	}

	QSizeF QBarsController3D::barSpacing()
	{
		return m_barSpacing;
	}

	bool QBarsController3D::isBarSpecRelative()
	{
		return m_isBarSpecRelative;
	}

	void QBarsController3D::setFloorLevel(float level)
	{
		m_floorLevel = level;
		m_isDataDirty = true;
	...
	float QBarsController3D::floorLevel() const
	{
		return m_floorLevel;
	}

	void QBarsController3D::setSelectionMode(QAbstractGraph3D::SelectionFlags mode)
	{
	...
	void QBarsController3D::setSelectedBar(const QPoint &position, QBarSeries3D *series, bool enterSlice)
	{
		...
		if (pos != m_selectedBar || series != m_selectedBarSeries) {
			...
			foreach(QAbstractSeries3D *otherSeries, m_seriesList) {
				QBarSeries3D *barSeries = static_cast<QBarSeries3D *>(otherSeries);
	...
	void QBarsController3D::clearSelection()
	{
		setSelectedBar(invalidSelectionPosition(), 0, false);
	}

	void QBarsController3D::adjustAxisRanges()
	{
		...
		if (adjustZ || adjustX || adjustY) {
			...
			if (adjustZ || adjustX) {
				for (int series = 0; series < seriesCount; series++) {
					const QBarSeries3D *barSeries =
						static_cast<QBarSeries3D *>(m_seriesList.at(series));
					if (barSeries->isVisible()) {
						const QBarDataProxy3D *proxy = barSeries->dataProxy();
						...
						if (adjustX && proxy) {
							const QBarDataArray3D *array = proxy->array();
							int columnCount = 0;
	...
	void QBarsController3D::adjustSelectionPosition(QPoint &pos, const QBarSeries3D *series)
	{
		const QBarDataProxy3D *proxy = 0;
		if (series)
	...
	QAbstractAxis3D *QBarsController3D::createDefaultAxis(QAbstractAxis3D::AxisOrientation orientation)
	{
