...
#define MIN_POWER 2
	...
	GLuint QUtils3D::getNearestPowerOfTwo(GLuint value)
	{
	...
	QVector4D QUtils3D::vectorFromColor(const QColor &color)
	{
		return QVector4D(color.redF(), color.greenF(), color.blueF(), color.alphaF());
	}

	QColor QUtils3D::colorFromVector(const QVector3D &colorVector)
	{
		return QColor(colorVector.x() * 255.0f, colorVector.y() * 255.0f,
			colorVector.z() * 255.0f, 255.0f);
	}

	QColor QUtils3D::colorFromVector(const QVector4D &colorVector)
	{
		return QColor(colorVector.x() * 255.0f, colorVector.y() * 255.0f,
			colorVector.z() * 255.0f, colorVector.w() * 255.0f);
	}

	QImage QUtils3D::printTextToImage(const QFont &font, const QString &text, const QColor &bgrColor,
		...
		bool borders, int maxLabelWidth)
	{
		...
		if (maxLabelWidth && (labelBackground || QUtils3D::isOpenGLES()))
		...
		do {
			...
			if (!maxTextureSize || (labelSize.width() <= maxTextureSize
				&& (labelSize.width() <= targetWidth || !QUtils3D::isOpenGLES()))) {
	...
	QVector4D QUtils3D::getSelection(QPoint mousepos, int height)
	{
	...
	QImage QUtils3D::getGradientImage(QLinearGradient &gradient)
	{
	...
	QUtils3D::ParamType QUtils3D::preParseFormat(const QString &format, QString &preStr, QString &postStr,
	...
	QUtils3D::ParamType QUtils3D::mapFormatCharToParamType(char formatSpec)
	{
	...
	QString QUtils3D::formatLabelSprintf(const QByteArray &format, QUtils3D::ParamType paramType, qreal value)
	{
		switch (paramType) {
		case ParamTypeInt:
	...
	QString QUtils3D::formatLabelLocalized(QUtils3D::ParamType paramType, qreal value,
	...
	QString QUtils3D::defaultLabelFormat()
	{
		static const QString defaultFormat(QStringLiteral("%.2f"));
		return defaultFormat;
	}

	float QUtils3D::wrapValue(float value, float min, float max)
	{
		if (value > max) {
			value = min + (value - max);
	...
	QQuaternion QUtils3D::calculateRotation(const QVector3D &xyzRotations)
	{
