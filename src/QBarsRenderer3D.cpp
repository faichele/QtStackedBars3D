...
namespace QtStackedBar3DVis
{
	...
	QBarsRenderer3D::QBarsRenderer3D(QBarsController3D *controller)
		...
		m_visualSelectedBarPos(QBarsController3D::invalidSelectionPosition()),
		m_selectedBarPos(QBarsController3D::invalidSelectionPosition()),
		...
		m_seriesStep(0.0f),
		m_seriesStart(0.0f),
		m_clickedPosition(QBarsController3D::invalidSelectionPosition()),
	...
	QBarsRenderer3D::~QBarsRenderer3D()
	{
	...
	void QBarsRenderer3D::initializeOpenGL()
	{
	...
	void QBarsRenderer3D::fixCameraTarget(QVector3D &target)
	{
	...
	void QBarsRenderer3D::getVisibleItemBounds(QVector3D &minBounds, QVector3D &maxBounds)
	{
	...
	void QBarsRenderer3D::updateData()
	{
		...
		foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
			QBarSeriesRenderCache3D *cache = static_cast<QBarSeriesRenderCache3D *>(baseCache);
			if (cache->isVisible()) {
				const QBarSeries3D *currentSeries = cache->series();
				...
				if (cache->dataDirty() || dimensionsChanged) {
					QBarDataProxy3D *dataProxy = currentSeries->dataProxy();
					...
					for (int i = 0; i < newRows; i++) {
						BarRenderItemRow3D &renderRow = renderArray[i];
						const QBarDataRow3D *dataRow = 0;
	...
	void QBarsRenderer3D::updateRenderRow(const QBarDataRow3D *dataRow, BarRenderItemRow3D &renderRow)
	{
		int j = 0;
	...
	void QBarsRenderer3D::updateRenderItem(const QBarDataItem3D &dataItem, QBarRenderItem3D &renderItem)
	{
	...
	void QBarsRenderer3D::updateSeries(const QList<QAbstractSeries3D *> &seriesList)
	{
		...
		for (int i = 0; i < seriesCount; i++) {
			QBarSeries3D *barSeries = static_cast<QBarSeries3D *>(seriesList[i]);
			QBarSeriesRenderCache3D *cache =
				static_cast<QBarSeriesRenderCache3D *>(m_renderCacheList.value(barSeries));
			if (barSeries->isVisible()) {
				if (noSelection
					&& barSeries->selectedBar() != QBarSeries3D::invalidSelectionPosition()) {
	...
	QSeriesRenderCache3D *QBarsRenderer3D::createNewCache(QAbstractSeries3D *series)
	{
		return new QBarSeriesRenderCache3D(series, this);
	}

	void QBarsRenderer3D::updateRows(const QVector<QBarsController3D::ChangeRow> &rows)
	{
		int minRow = m_axisCacheZ.min();
		int maxRow = m_axisCacheZ.max();
		QBarSeriesRenderCache3D *cache = 0;
		const QBarSeries3D *prevSeries = 0;
		const QBarDataArray3D *dataArray = 0;

		foreach(QBarsController3D::ChangeRow item, rows) {
			const int row = item.row;
			if (row < minRow || row > maxRow)
				continue;
			QBarSeries3D *currentSeries = item.series;
			if (currentSeries != prevSeries) {
				cache = static_cast<QBarSeriesRenderCache3D *>(m_renderCacheList.value(currentSeries));
	...
	void QBarsRenderer3D::updateItems(const QVector<QBarsController3D::ChangeItem> &items)
	{
		int minRow = m_axisCacheZ.min();
		int maxRow = m_axisCacheZ.max();
		int minCol = m_axisCacheX.min();
		...
		foreach(QBarsController3D::ChangeItem item, items) {
	...
	void QBarsRenderer3D::updateScene(QScene3D *scene)
	{
		if (!m_noZeroInRange) {
	...
	void QBarsRenderer3D::render(GLuint defaultFboHandle)
	{
	...
	void QBarsRenderer3D::drawSlicedScene()
	{
		...
		if (!sliceGridLabels) {
					...
					for (int col = 0; col < sliceCount; col++) {
						...
						if (item.height() != 0.0f || (!m_noZeroInRange && item.value() == 0.0f)) {
							...
							QDrawer3D::LabelPosition labelPos =
								(item.height() < 0) ? QDrawer3D::LabelBelow : QDrawer3D::LabelOver;
							m_dummyBarRenderItem.setTranslation(QVector3D(item.translation().x(),
								barPosYAdjustment
								- zeroPosAdjustment
								+ item.height(),
			...
			QDrawer3D::LabelPosition labelPos =
				(selectedItem->height() < 0) ? QDrawer3D::LabelBelow : QDrawer3D::LabelOver;
	...
	void QBarsRenderer3D::drawScene(GLuint defaultFboHandle)
	{
		GLint startBar = 0;
		GLint stopBar = 0;
		GLint stepBar = 0;
		...
		QBarRenderItem3D *selectedBar(0);
			...
			foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
				if (baseCache->isVisible()) {
					...
					for (int row = startRow; row != stopRow; row += stepRow) {
						const BarRenderItemRow3D &renderRow = renderArray.at(row);
						for (int bar = startBar; bar != stopBar; bar += stepBar) {
							const QBarRenderItem3D &item = renderRow.at(bar);
							if (!item.value())
								continue;
		...
		if (barSelectionFound) {
			...
			QDrawer3D::LabelPosition position =
				m_selectedBar->height() >= 0 ? QDrawer3D::LabelOver : QDrawer3D::LabelBelow;
	...
	bool QBarsRenderer3D::drawBars(QBarRenderItem3D **selectedBar,
		...
		GLint startBar, GLint stopBar, GLint stepBar, GLfloat reflection)
	{
		...
		foreach(QSeriesRenderCache3D *baseCache, m_renderCacheList) {
			if (baseCache->isVisible()) {
				...
				previousColorStyle = colorStyle;
				for (int row = startRow; row != stopRow; row += stepRow) {
					BarRenderItemRow3D &renderRow = renderArray[row];
					for (int bar = startBar; bar != stopBar; bar += stepBar) {
						QBarRenderItem3D &item = renderRow[bar];
						...
						if (m_cachedSelectionMode > QAbstractGraph3D::SelectionNone) {
							QBarsController3D::SelectionType selectionType =
								QBarsController3D::SelectionNone;
							if (somethingSelected)
								selectionType = isSelected(row, bar, cache);

							switch (selectionType) {
							case QBarsController3D::SelectionItem: {
								if (colorStyleIsUniform)
							...
							case QBarsController3D::SelectionRow: {
							...
							case QBarsController3D::SelectionColumn: {
							...
							case QBarsController3D::SelectionNone: {
	...
	void QBarsRenderer3D::drawBackground(GLfloat backgroundRotation,
		const QMatrix4x4 &depthProjectionViewMatrix,
	...
	void QBarsRenderer3D::drawGridLines(const QMatrix4x4 &depthProjectionViewMatrix,
		const QMatrix4x4 &projectionViewMatrix,
	...
	void QBarsRenderer3D::drawLabels(bool drawSelection, const QCamera3D *activeCamera,
	...
	void QBarsRenderer3D::updateMultiSeriesScaling(bool uniform)
	{
	...
	void QBarsRenderer3D::updateBarSpecs(GLfloat thicknessRatio, const QSizeF &spacing, bool relative)
	{
	...
	void QBarsRenderer3D::updateAxisRange(QAbstractAxis3D::AxisOrientation orientation, float min,
		float max)
	{
	...
	void QBarsRenderer3D::updateAxisReversed(QAbstractAxis3D::AxisOrientation orientation, bool enable)
	{
	...
	void QBarsRenderer3D::updateSelectedBar(const QPoint &position, QBarSeries3D *series)
	{
		m_selectedBarPos = position;
		m_selectedSeriesCache = static_cast<QBarSeriesRenderCache3D *>(m_renderCacheList.value(series, 0));
		m_selectionDirty = true;
		m_selectionLabelDirty = true;
		...
		if (m_selectedBarPos == QBarsController3D::invalidSelectionPosition()
	...
	void QBarsRenderer3D::resetClickedStatus()
	{
		m_clickedPosition = QBarsController3D::invalidSelectionPosition();
		m_clickedSeries = 0;
	}

	void QBarsRenderer3D::updateShadowQuality(QAbstractGraph3D::ShadowQuality quality)
	{
	...
	void QBarsRenderer3D::fixMeshFileName(QString &fileName, QAbstractSeries3D::Mesh mesh)
	{
	...
	void QBarsRenderer3D::calculateSceneScalingFactors()
	{
	...
	void QBarsRenderer3D::calculateHeightAdjustment()
	{
	...
	QBarsController3D::SelectionType QBarsRenderer3D::isSelected(int row, int bar,
		const QBarSeriesRenderCache3D *cache)
	{
		QBarsController3D::SelectionType isSelectedType = QBarsController3D::SelectionNone;
		...
		if ((m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionMultiSeries)
			...
			if (row == m_visualSelectedBarPos.x() && bar == m_visualSelectedBarPos.y()
				...
				isSelectedType = QBarsController3D::SelectionItem;
			}
			else if (row == m_visualSelectedBarPos.x()
				&& (m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionRow))) {
				isSelectedType = QBarsController3D::SelectionRow;
			}
			else if (bar == m_visualSelectedBarPos.y()
				&& (m_cachedSelectionMode.testFlag(QAbstractGraph3D::SelectionColumn))) {
				isSelectedType = QBarsController3D::SelectionColumn;
			}
		}
	...
	QPoint QBarsRenderer3D::selectionColorToArrayPosition(const QVector4D &selectionColor)
	{
		QPoint position = QBarsController3D::invalidSelectionPosition();
		...
		if (selectionColor.w() == itemAlpha) {
			...
			// Value selection
			position = QBarsController3D::invalidSelectionPosition();
	...
	QBarSeries3D *QBarsRenderer3D::selectionColorToSeries(const QVector4D &selectionColor)
	{
	...
	void QBarsRenderer3D::updateSlicingActive(bool isSlicing)
	{
	...
	void QBarsRenderer3D::initShaders(const QString &vertexShader, const QString &fragmentShader)
	{
		if (m_barShader)
			delete m_barShader;
	...
	void QBarsRenderer3D::initGradientShaders(const QString &vertexShader, const QString &fragmentShader)
	{
	...
	void QBarsRenderer3D::initSelectionShader()
	{
	...
	void QBarsRenderer3D::initSelectionBuffer()
	{
		m_textureHelper->deleteTexture(&m_selectionTexture);
	...
	void QBarsRenderer3D::initDepthShader()
	{
		if (!m_isOpenGLES) {
			if (m_depthShader)
				delete m_depthShader;
	...
	void QBarsRenderer3D::updateDepthBuffer()
	{
		if (!m_isOpenGLES) {
	...
	void QBarsRenderer3D::initBackgroundShaders(const QString &vertexShader,
	...
	QVector3D QBarsRenderer3D::convertPositionToTranslation(const QVector3D &position, bool isAbsolute)
	{
		float xTrans = 0.0f;
		float yTrans = 0.0f;
		float zTrans = 0.0f;
	...
	void QBarsRenderer3D::updateAspectRatio(float ratio)
	{
		Q_UNUSED(ratio)
	}

	void QBarsRenderer3D::updateFloorLevel(float level)
	{
	...
	void QBarsRenderer3D::updateMargin(float margin)
	{
