...
namespace QtStackedBar3DVis
{
	...
	QObjectHelper3D::QObjectHelper3D(const QString &objectFile)
	...
	// The "QAbstractRenderer3D *" key identifies the renderer
	static QHash<const QAbstractRenderer3D *, QHash<QString, ObjectHelperRef *> *> cacheTable;

	QObjectHelper3D::~QObjectHelper3D()
	{
	}

	void QObjectHelper3D::resetObjectHelper(const QAbstractRenderer3D *cacheId, QObjectHelper3D *&obj,
	...
	void QObjectHelper3D::releaseObjectHelper(const QAbstractRenderer3D *cacheId, QObjectHelper3D *&obj)
	{
		Q_ASSERT(cacheId);
	...
	QObjectHelper3D *QObjectHelper3D::getObjectHelper(const QAbstractRenderer3D *cacheId,
		const QString &objectFile)
	{
		...
		if (!objRef) {
			...
			objRef->refCount = 0;
			objRef->obj = new QObjectHelper3D(objectFile);
	...
	void QObjectHelper3D::load()
	{
		...
		bool loadOk = QMeshLoader3D::loadOBJ(m_objectFile, vertices, uvs, normals);
		if (!loadOk)
			qFatal("loading failed");

		// Index vertices
		QVertexIndexer3D::indexVBO(vertices, uvs, normals, m_indices, m_indexedVertices, m_indexedUVs,
			m_indexedNormals);
