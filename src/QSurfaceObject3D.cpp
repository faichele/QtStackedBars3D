...
namespace QtStackedBar3DVis
{
	...
	QSurfaceObject3D::QSurfaceObject3D(QSurfaceRenderer3D *renderer)
	...
	QSurfaceObject3D::~QSurfaceObject3D()
	{
	...
	void QSurfaceObject3D::setUpSmoothData(const QSurfaceDataArray3D &dataArray, const QRect &space,
		bool changeGeometry, bool polar, bool flipXZ)
	{
		...
		for (int i = 0; i < m_rows; i++) {
			const QSurfaceDataRow3D &p = *dataArray.at(i);
	...
	void QSurfaceObject3D::createSmoothNormalBodyLine(int &totalIndex, int column)
	{
		int colLimit = m_columns - 1;
	...
	void QSurfaceObject3D::createSmoothNormalUpperLine(int &totalIndex)
	{
		if (m_dataDimension == BothAscending) {
	...
	QVector3D QSurfaceObject3D::createSmoothNormalBodyLineItem(int x, int y)
	{
		int p = y * m_columns + x;
		if (m_dataDimension == BothAscending) {
			if (x < m_columns - 1) {
	...
	QVector3D QSurfaceObject3D::createSmoothNormalUpperLineItem(int x, int y)
	{
		int p = y * m_columns + x;
		if (m_dataDimension == BothAscending) {
			if (x < m_columns - 1) {
	...
	void QSurfaceObject3D::smoothUVs(const QSurfaceDataArray3D &dataArray,
		const QSurfaceDataArray3D &modelArray)
	{
		...
		const bool zDescending = m_dataDimension.testFlag(QSurfaceObject3D::ZDescending);
		const bool xDescending = m_dataDimension.testFlag(QSurfaceObject3D::XDescending);
		...
		for (int i = 0; i < m_rows; i++) {
			...
			if (zDescending)
				y = 1.0f - y;
			const QSurfaceDataRow3D &p = *modelArray.at(i);
			for (int j = 0; j < m_columns; j++) {
	...
	void QSurfaceObject3D::updateSmoothRow(const QSurfaceDataArray3D &dataArray, int rowIndex, bool polar)
	{
		// Update vertices
		int p = rowIndex * m_columns;
		const QSurfaceDataRow3D &dataRow = *dataArray.at(rowIndex);
	...
	void QSurfaceObject3D::updateSmoothItem(const QSurfaceDataArray3D &dataArray, int row, int column,
		bool polar)
	{
		// Update a vertice
	...
	void QSurfaceObject3D::createSmoothIndices(int x, int y, int endX, int endY)
	{
		if (endX >= m_columns)
			endX = m_columns - 1;
		if (endY >= m_rows)
	...
	void QSurfaceObject3D::createSmoothGridlineIndices(int x, int y, int endX, int endY)
	{
		if (endX >= m_columns)
			endX = m_columns - 1;
		if (endY >= m_rows)
	...
	void QSurfaceObject3D::setUpData(const QSurfaceDataArray3D &dataArray, const QRect &space,
		bool changeGeometry, bool polar, bool flipXZ)
	{
		m_columns = space.width();
		m_rows = space.height();
		...
		for (int i = 0; i < m_rows; i++) {
			const QSurfaceDataRow3D &row = *dataArray.at(i);
			for (int j = 0; j < m_columns; j++) {
	...
	void QSurfaceObject3D::coarseUVs(const QSurfaceDataArray3D &dataArray,
		const QSurfaceDataArray3D &modelArray)
	{
		if (dataArray.size() == 0 || modelArray.size() == 0)
			return;
	...
	void QSurfaceObject3D::updateCoarseRow(const QSurfaceDataArray3D &dataArray, int rowIndex, bool polar)
	{
		int colLimit = m_columns - 1;
		int doubleColumns = m_columns * 2 - 2;
	...
	void QSurfaceObject3D::updateCoarseItem(const QSurfaceDataArray3D &dataArray, int row, int column,
		bool polar)
	{
		int colLimit = m_columns - 1;
		int doubleColumns = m_columns * 2 - 2;
	...
	void QSurfaceObject3D::createCoarseSubSection(int x, int y, int columns, int rows)
	{
	...
	void QSurfaceObject3D::createCoarseGridlineIndices(int x, int y, int endX, int endY)
	{
		if (endX >= m_columns)
			endX = m_columns - 1;
		if (endY >= m_rows)
	...
	void QSurfaceObject3D::uploadBuffers()
	{
		QVector<QVector2D> uvs; // Empty dummy
		createBuffers(m_vertices, uvs, m_normals, 0);
	}

	void QSurfaceObject3D::createBuffers(const QVector<QVector3D> &vertices, const QVector<QVector2D> &uvs,
	...
	void QSurfaceObject3D::checkDirections(const QSurfaceDataArray3D &array)
	{
	...
	void QSurfaceObject3D::getNormalizedVertex(const QSurfaceDataItem3D &data, QVector3D &vertex,
	...
	GLuint QSurfaceObject3D::gridElementBuf()
	{
		if (!m_meshDataLoaded)
	...
	GLuint QSurfaceObject3D::gridIndexCount()
	{
		return m_gridIndexCount;
	}

	QVector3D QSurfaceObject3D::vertexAt(int column, int row)
	{
		int pos = 0;
	...
	void QSurfaceObject3D::createCoarseIndices(GLint *indices, int &p, int row, int upperRow, int j)
	{
		if ((m_dataDimension == BothAscending) || (m_dataDimension == BothDescending)) {
			// Left triangle
			indices[p++] = row + j + 1;
	...
	void QSurfaceObject3D::createNormals(int &p, int row, int upperRow, int j)
	{
		if ((m_dataDimension == BothAscending) || (m_dataDimension == BothDescending)) {
	...
	QVector3D QSurfaceObject3D::normal(const QVector3D &a, const QVector3D &b, const QVector3D &c)
	{
		QVector3D v1 = b - a;
		QVector3D v2 = c - a;
