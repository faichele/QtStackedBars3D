...
namespace QtStackedBar3DVis
{
	...
	QAxisRenderCache3D::QAxisRenderCache3D()
		: m_type(QAbstractAxis3D::AxisTypeNone),
		m_min(0.0f),
		m_max(10.0f),
		m_segmentCount(5),
		m_subSegmentCount(1),
	...
	QAxisRenderCache3D::~QAxisRenderCache3D()
	{
		foreach(QLabelItem3D *label, m_labelItems)
			delete label;
		m_titleItem.clear();
	...
	void QAxisRenderCache3D::setDrawer(QDrawer3D *drawer)
	{
		m_drawer = drawer;
	...
	void QAxisRenderCache3D::setType(QAbstractAxis3D::AxisType type)
	{
		m_type = type;
	...
	void QAxisRenderCache3D::setTitle(const QString &title)
	{
		if (m_title != title) {
			m_title = title;
	...
	void QAxisRenderCache3D::setLabels(const QStringList &labels)
	{
		if (m_labels != labels) {
			...
			for (int i = 0; i < newSize; i++) {
				if (i >= oldSize)
					m_labelItems.append(new QLabelItem3D);
				if (m_drawer) {
	...
	void QAxisRenderCache3D::updateAllPositions()
	{
	...
	void QAxisRenderCache3D::updateTextures()
	{
		m_font = m_drawer->font();
	...
	void QAxisRenderCache3D::clearLabels()
	{
		m_titleItem.clear();
		for (int i = 0; i < m_labels.size(); i++)
			m_labelItems[i]->clear();
	...
	int QAxisRenderCache3D::maxLabelWidth(const QStringList &labels) const
	{
		int labelWidth = 0;
