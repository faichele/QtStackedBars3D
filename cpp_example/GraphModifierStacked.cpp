...
GraphModifierStacked::GraphModifierStacked(QStackedBars3D *bargraph)
	: GraphModifierBase(bargraph),
	m_graph(bargraph),
	m_primarySeries(new QStackedBarSeries3D),
	m_secondarySeries(new QStackedBarSeries3D),
...
GraphModifierStacked::~GraphModifierStacked()
{
	delete m_graph;
}

void GraphModifierStacked::changeStyle(int style)
{
...
void GraphModifierStacked::zoomToSelectedBar()
{
	...
	if (selectedBar != QBarSeries3D::invalidSelectionPosition()) {
		...
		if (endTarget.x() > 0.0f)
			endAngleX -= 180.0f;
		float barValue = 0.0f;
		if (m_graph->selectedSeries()->dataProxy()->itemAt(selectedBar.x(),
			selectedBar.y())->values().size() > 0)
		{
			m_graph->selectedSeries()->dataProxy()->itemAt(selectedBar.x(),
				selectedBar.y())->values().at(0);
		}
...
void GraphModifierStacked::setSmoothBars(int smooth)
{
...
void GraphModifierStacked::setSeriesVisibility(int enabled)
{
	m_secondarySeries->setVisible(bool(enabled));
}

void GraphModifierStacked::setReverseValueAxis(int enabled)
{
	m_graph->valueAxis()->setReversed(enabled);
}

void GraphModifierStacked::resetCycleDeviationData()
{
	// Set up data
	...
	// Create data arrays
	QStackedBarDataArray3D *dataSet = new QStackedBarDataArray3D;
	QStackedBarDataArray3D *dataSet2 = new QStackedBarDataArray3D;
	QStackedBarDataRow3D *dataRow;
	...
	for (int cycle_1 = 0; cycle_1 < m_cycleCount; cycle_1++) {
		...
		for (int cycle_2 = 0; cycle_2 < m_cycleCount; cycle_2++) {
			...
			else
			{
				...
				(*dataRow)[cycle_2].setValues(rnd_values);
				(*dataRow2)[cycle_2].setValues(rnd_values);
			}
		}
	...
	sectionColors << QColor(220, 20, 20, 128) << QColor(20, 220, 20, 128) << QColor(20, 20, 220, 255) << QColor(220, 220, 20, 255) << QColor(220, 20, 220, 255) << QColor(20, 220, 220, 255);

	m_primarySeries->setSectionColors(sectionColors);
	m_secondarySeries->setSectionColors(sectionColors);
}